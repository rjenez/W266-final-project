...
template <typename TipElemenata>
 ...
 int sirina_ispisa,const int preciznost=6,bool treba_brisati=false) {
 for(int i = 0; i < mat.br_redova; i++) {
 for(int j = 0; j < mat.br_kolona; j++)
...
template <typename TipElemenata>
 ...
 template<typename TipElemenata>
 Matrica<TipElemenata>Mnozenje(Matrica<TipElemenata>mat,double k){
     for(int i=0;i<mat.br_redova;i++){
         for(int j=0;j<mat.br_kolona;j++){
             mat.elementi[i][j]=mat.elementi[i][j]*k;
         }
     }
     return mat;
 }
 ...
 Matrica<TipElemenata>MatricniPolinom(Matrica<TipElemenata>A,vector<double>v){
     ...
     Matrica<TipElemenata>Prosla=Jedinicna(StvoriMatricu<TipElemenata>(A.br_redova,A.br_kolona));
     ...
     for(int i=0;i<v.size();i++){
         Suma=ZbirMatrica(Suma,Mnozenje(ProduktMatrica(Prosla,A),v[i]));
         Prosla=ProduktMatrica(Prosla,A);
     }
