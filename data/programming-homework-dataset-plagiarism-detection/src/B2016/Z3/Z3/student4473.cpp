...
template <typename TipElemenata>
Matrica<TipElemenata> ProduktMatrica(const Matrica<TipElemenata> &m1, cont Matrica<TipElemenata> &m2){
    ...
    for(int i=0;i<m3.br_redova;i++){
        for(int j=0;j<m3.br_kolona;j++){
            for(int k=0;k<m1.br_redova;k++)
                 m3.elementi[i][j]+=m1.elementi[i][k]*m2[k][j];
        }
    }
    return m3;
}
...
template <typename TipElemenata>
MatricniPolinom (const Matrica<TipElemenata> &a, std::vector<int> v){
...
{   Matrica<double> a,b,c;
    ...
    try{
        ...
        UnesiMatricu('B',b);
        std::cout << "Zbir ove dvije matrice je:" << std::endl;
