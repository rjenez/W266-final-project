...
Matrica<TipElemenata>MatricniPolinom(const Matrica<TipElemenata>&m1, std::vector<double>v){
    ...
    if(v.size()==0){
        auto nulta=StvoriMatricu<TipElemenata>(m1.br_redova,m1.br_kolona);
       /* for(int i=0;i<nulta.br_redova;i++){
            for(int j=0;j<nulta.br_kolona;j++){
    ...
    for(int i=0;i<m1.br_redova;i++){
        for(int j=0;j<m1.br_kolona;j++){
            mat.elementi[i][j]=m1.elementi[i][j];
        }
    }
    ...
    for(int k=1;k<v.size();k++){
        ...
        for(int i=0;i<m1.br_redova;i++){
            for(int j=0;j<m1.br_kolona;j++){
                mat.elementi[i][j]*=v[k];
            }
        }
        m2=ZbirMatrica(m2,mat);
    }
    UnistiMatricu(mat);
    return m2;
}
