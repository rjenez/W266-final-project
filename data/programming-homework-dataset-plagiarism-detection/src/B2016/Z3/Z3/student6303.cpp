...
{
    if(!dobar(m1.br_kolona,m2.br_redova)) throw domain_error("Matrice nisu saglasne za mnozenje");
    auto r=nule<TipElemenata>(m1.br_redova,m2.br_kolona);
    for(int i=0; i<m1.br_redova; i++)
        for(int j=0; j<m2.br_kolona; j++)
            for(int k=0; k<m2.br_redova; k++)
                r.elementi[i][j]+=m1.elementi[i][k]*m2.elementi[k][j];
    return r;
}
