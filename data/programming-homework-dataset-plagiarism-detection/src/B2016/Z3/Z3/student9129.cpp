...
void IspisiMatricu(Matrica<TipElemenata> mat,
                   int sirina_ispisa, int preciznost = 6, bool trebabrisati = false)
{
    for(int i = 0; i < mat.br_redova; i++)
    {
        for(int j = 0; j < mat.br_kolona; j++)
...
{
    if(m1.br_kolona != m2.br_kolona) throw std::domain_error("Matrice nisu saglasne za mnozenje");
