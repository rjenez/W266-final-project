...
VektorSet Razvrstavanje(std::vector<std::string> imena, int broj_ekipa){
    ...
    VektorSet rezultat;
    std::shared_ptr<Dijete> pocetak(nullptr), trenutni(nullptr) , predhodni(nullptr) ;
    ...
    for (std::shared_ptr<Dijete> p = pocetak ; velicina != 0 ; p = p->sljedeci ) {
        br++;
        if(br == 1 && velicina == imena.size()){
            std::shared_ptr<Dijete> temp = p;
        ...
        if (br == korak - 1) {
            std::shared_ptr<Dijete> temp = p->sljedeci;
