...
typedef std::shared_ptr<Dijete> PametniNaDijete;
...
PametniNaDijete Vrti(PametniNaDijete pocetak, PametniNaDijete &prethodni, int duzina){
...
std::vector<std::set<std::string>> Razvrstavanje(std::vector<std::string> Imena, int broj_timova){
    ...
    PametniNaDijete pocetak = nullptr, prethodni;
    int indeks(0);
    for(;;){
        PametniNaDijete novi(std::make_shared<Dijete>(Dijete()));
    ...
    PametniNaDijete Obrisi_me = nullptr;
    for(int i(0); i < broj_timova; i++){
        ...
        for(int j(0); j < velicina; j++){
            ...
            if(broj_djece == 1) pocetak -> sljedeci = nullptr;
            Obrisi_me = nullptr;
            broj_djece--;
        }
    }
