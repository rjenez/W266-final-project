...
struct Djete{
    std::string ime;
    std::shared_ptr<Djete> sljedeci;
};
typedef std::shared_ptr<Djete> Pametni;
...
std::vector<std::set<std::string>> Razvrstavanje(std::vector<std::string> v, int broj_timova){
    ...
    Pametni pocetno(nullptr), prethodno;
    for(int i=0;i<v.size();i++){
        Pametni novi(std::make_shared<Djete>());
        novi->ime=v[i];
