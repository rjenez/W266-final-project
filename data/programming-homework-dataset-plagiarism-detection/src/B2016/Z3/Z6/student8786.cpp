...
{
    string ime;
    shared_ptr<Dijete>slj;
};

shared_ptr<Dijete>transformisi(vector<string> v)
{
    shared_ptr<Dijete>p=nullptr;
    shared_ptr<Dijete>t=nullptr;
    for(int i=0; i<v.size(); i++)
    {
        shared_ptr<Dijete>n= make_shared<Dijete>();
        n->ime=v[i];
        if(i==0) { p=n; t=n;}
        else { t->slj=n; t=n;}
...
vector<set<string>> Razvrstavanje(vector<string> v, int n)
{
    ...
    shared_ptr<Dijete> li=transformisi(v);
    shared_ptr<Dijete> pr=li;
    shared_ptr<Dijete> l=li;
    ...
    while(brt1!=0)
    {
        int a=brc1;
        set<string> skup;
        if(nes){ nes=false; skup.insert(l->ime); pr->slj=l->slj; l->slj=nullptr;  l=pr->slj; a--; brs--;}
        while(a>0)
        {
            ...
            skup.insert(l->ime);
            brs=slova(l->ime);
            if(brt1==1 && a==1 && brt2==0) { l->slj=nullptr; }
            else { pr->slj=l->slj; l->slj=nullptr;  l=pr->slj;}
            brs--;
            a--;
        }
        rez.push_back(skup);
