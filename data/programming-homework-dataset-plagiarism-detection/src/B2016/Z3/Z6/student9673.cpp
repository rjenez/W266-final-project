...
std::shared_ptr<Dijete> NapraviListu(std::vector<std::string> v){
    int n(0);
    std::shared_ptr<Dijete> pocetak(nullptr), prosli(nullptr);
...
void UnistiListu(std::shared_ptr<Dijete> pocetak) {
        std::shared_ptr<Dijete> novi(pocetak);
        while (novi != nullptr) { novi = pocetak -> sljedeci; pocetak = nullptr; pocetak = novi; }
    }
...
std::vector<std::set<std::string>> Razvrstavanje(std::vector<std::string> s, const int k){
    ...
    std::shared_ptr<Dijete> lista, prosli, kraj;
    ...
    while(1){
        ...
        if(ProvjeriTimove(timovi, velicine)){ 
