...
std::vector<std::set<std::string>> Razvrstavanje(std::vector<std::string> djeca,int brojtimova)
{
    ...
    std::shared_ptr<Dijete> djecica=std::shared_ptr<Dijete>(new Dijete[djeca.size()]);
    ...
    int trenutni_skup=0;
    std::shared_ptr<Dijete> trenutno_dijete=djecica;
    std::shared_ptr<Dijete> tr_dva=djecica+(djeca.size())-1;
