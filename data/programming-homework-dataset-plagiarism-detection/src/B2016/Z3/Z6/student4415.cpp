...
std::vector<std::set<std::string>> Razvrstavanje (std::vector<std::string> djeca2, int br_tim)
{
    ...
    for (auto &i : djeca2) {
        ...
        else prethodni-> sljedeci = std::shared_ptr<Dijete>(novi);
        prethodni = novi;
    }
    prethodni->sljedeci = std::shared_ptr<Dijete>(pocetak);
