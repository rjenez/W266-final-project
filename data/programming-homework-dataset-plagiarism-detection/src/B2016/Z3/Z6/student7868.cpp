...
std::vector<std::set<std::string>> Razvrstavanje(std::vector<std::string> Imena, int broj_timova){
    ...
    try{
        std::shared_ptr<Dijete> zadnji{nullptr};
        for(int i{0}; i<Imena.size(); i++){
            std::shared_ptr<Dijete> dijete(new Dijete);
            dijete->ime=Imena[i];
            dijete->sljedeci=nullptr;
            if(!pocetak) pocetak=dijete;
            dijete->sljedeci=pocetak;
            if(i!=0)zadnji->sljedeci=dijete;
            zadnji=dijete;
        }
        prethodni=zadnji;
    }
    ...
    int preskoci{0}, j{1};
    std::shared_ptr<Dijete> kreci{pocetak}, poslije;
