...
int BrojElemenata(Dijete *pocetak)
{
    if(!pocetak)  return 0; 
    if(pocetak==pocetak->sljedeci) return 1;
    int broj_el{0};
    bool naPocetkuSmo{true};
    for(Dijete *p=pocetak; p!=pocetak || naPocetkuSmo;p=p->sljedeci)
    {
        broj_el++;
        naPocetkuSmo=false;
    }
...
void UnistiCvor(Dijete* &prijasnji)
{
    //Dijete *prijasnjiCvor{cvor->prijasnji};
    if(prijasnji==prijasnji->sljedeci)   { delete prijasnji; prijasnji=nullptr; return ; }
    Dijete *priv{prijasnji->sljedeci};
    prijasnji->sljedeci= prijasnji->sljedeci->sljedeci;
    //priv->sljedeci->prijasnji=prijasnjiCvor;
    //priv->sljedeci=nullptr;
    delete priv;
    priv=nullptr;
}
...
{
    if(br_timova<1 || (unsigned int)br_timova>imena_djece.size()) throw logic_error("Izuzetak: Razvrstavanje nemoguce");
    ...
    Dijete *pocetak(nullptr), *trenutni{nullptr};
    ...
    while(brojacManjih<brojManjihTimova)
    {
        ...
        for(int velSkupa=0;velSkupa<br_manjiTim;velSkupa++)
        {
            ...
            //if(BrojElemenata(it)==0) break;
            int pomaci {duzinaStringa(trenutnoDijete) };
            if(!it) { break; }   //dodano zadnje
            
            for(int i=1;i<pomaci;i++)  { it=it->sljedeci; } // cout<<" "<<BrojElemenata(it)<<" ";} 
            trenutnoDijete = it->sljedeci->ime;
        }
        brojacManjih++;
        rez.push_back(a);
