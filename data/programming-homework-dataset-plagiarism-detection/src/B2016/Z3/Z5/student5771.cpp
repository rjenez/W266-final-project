...
Dijete *KreirajPovezanuListu(vector<string> djeca){
    Dijete *pocetak=nullptr;
    Dijete *prethodni;
    for(int i=0; i<djeca.size(); i++){
        Dijete *novi(new Dijete);//pravi//
...
vector<set<string>>Razvrstavanje(vector<string> djeca, int n){
 ...
 vector<set<string>> tim;
    ...
    if(brojackrugova==brojslovaproslog){
    	...
    	if(brojubacenih!=djeca.size())it=it->sljedeci; brojackrugova++;
    }
...
int main ()
{
    ...
    IspisiTimove(timovi);}
    catch(bad_alloc izuzetak){ cout<<"Nedovoljno memorije"; }
