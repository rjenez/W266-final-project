...
std::map<std::string,std::set<int>> KreirajIndeksPojmova(std::string tekst){
    ...
    for(auto izdvojena: spisak_rijeci){
        ...
        while(iter2!=tekst.end()){
            ...
            if(istina==false)
                pom--;
            if(iter1!=izdvojena.end() && iter2!=tekst.end() && ((istina==true && *iter1==*iter2) || (istina==false && (*pom==' ' || *pom<'0' || (*pom>'9' && *pom<'A') || (*pom>'Z' && *pom<'a') || *pom>'z')))){
                ...
                while(iter1_pomoc!=izdvojena.end() && iter2_pomoc!=tekst.end() && *iter1_pomoc==*iter2_pomoc){
                    ...
                    if(iter1_pomoc==izdvojena.end() && (iter2_pomoc==tekst.end() || (*iter2_pomoc==' ' || *iter2_pomoc<'0' || (*iter2_pomoc>'9' && *iter2_pomoc<'A') || (*iter2_pomoc>'Z' && *iter2_pomoc<'a') || *iter2_pomoc>'z')))
                        pozicija.insert(i);
                }
            }
            istina=false;
