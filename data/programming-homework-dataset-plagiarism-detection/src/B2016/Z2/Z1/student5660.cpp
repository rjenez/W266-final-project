...
Tabla KreirajIgru(int n, const std::vector<std::vector<int>> &mine){
		...
		return matrica;
	//else throw std::domain_error("Broj polja ne moze biti negativan!"); //popravi
}
std::vector<std::vector<int>> PrikaziOkolinu(const Tabla &polja, int x, int y){
	...
	for(int xo=x-1; xo<=x+1;xo++){
		for(int yo=y-1;yo<=y+1;yo++){
			//sve tacke iz okoline x,y
			//brojanje mina oko tacke xo yo
			brmina=0;
...
Status Idi(Tabla &polja, int &x, int &y, Smjerovi smjer){
	...
	//if(polja[x1][y1]==Polje::Prazno)return Status::KrajPobjeda;
...
bool UnosKomande(Komande &komanda, Smjerovi &smjer, int &x, int &y,KodoviGresaka &greska){
	...
	switch(ulaz[i]){
		...
		case 'B':{
			...
			int y1(0);
			int znak(1);
			while(i<ulaz.size() && jelic(ulaz[i])){
				if(ulaz[i]=='-')znak=-1;
				else{
					x1*=10;
					x1+=ulaz[i]-'0';
			...
			while(i<ulaz.size() && ulaz[i]==' ')i++;
			if(i!=ulaz.size()){
				greska=KodoviGresaka::SuvisanParametar;//SUVISAN
				return false;
			}
		...
		case 'D':{
			i++;
			//bool imabarjedanrazmak(false);
			while(i<ulaz.size() && ulaz[i]==' '){/*imabarjedanrazmak=true;*/i++;}
			/*if(!imabarjedanrazmak){
				greska=KodoviGresaka::PogresnaKomanda;
				return false;
			}*/
			if(i==ulaz.size()){
		...
		case 'P':{
			...
			if(ulaz[i]=='1'){
				...
				while(i<ulaz.size() && ulaz[i]==' ') i++;
				if(i!=ulaz.size()){
					greska=KodoviGresaka::SuvisanParametar; //suvisan param
					return false;
				}
