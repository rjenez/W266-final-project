...
class Biblioteka
{
    ...
    public:
        Biblioteka() { auto it1(mapa_korisnika.begin()); it1->second=nullptr; auto it2(mapa_knjiga.begin()); it2->second=nullptr; }
        ~Biblioteka();
...
void Biblioteka::RegistrirajNovuKnjigu(int ev_broj, string naslov, string ime, string zanr, int godina)
{
	...
	auto pok(new Knjiga(k));
	mapa_knjiga[ev_broj]=pok;
}
