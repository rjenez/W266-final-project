...
public:
    ...
    Biblioteka(const Biblioteka &biblio){
        for(auto p=biblio.mapakorisnika.begin();p!=biblio.mapakorisnika.end();p++){
            mapakorisnika[p->first]=new Korisnik{p->second->ime,p->second->prezime, p->second->adresa, p->second->telefon};
        }
        
        for(auto p=biblio.mapaknjiga.begin();p!=biblio.mapaknjiga.end();p++){
            mapaknjiga[p->first]=new Knjiga(p->second->DajNaslov(), p->second->DajAutora(), p->second->DajZanr(), p->second->DajGodinuIzdavanja());
    ...
    Biblioteka &operator =(const Biblioteka &biblio){
        if(&biblio!=this){
            for(auto p=mapaknjiga.begin();p!=mapaknjiga.end();p++) delete p->second;
            for(auto p=mapakorisnika.begin();p!=mapakorisnika.end();p++) delete p->second;
    ...
    if(!mapakorisnika.count(clanskibr)){
        Korisnik *p=new Korisnik{ime, prezime, adresa, telefon};
...
void Biblioteka::RegistrirajNovuKnjigu(int evidbroj, std::string nas, std::string pis, std::string tip, int godina){
    ...
    Knjiga *p=new Knjiga(nas, pis, tip, godina);    
