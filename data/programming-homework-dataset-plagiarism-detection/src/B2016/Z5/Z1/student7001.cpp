...
Biblioteka::Biblioteka(const Biblioteka &b) {
    try {
        for (auto it=b.mapa_korisnika.begin(); it!=b.mapa_korisnika.end(); it++) {
            Korisnik *pok=new Korisnik{it->second->ime,it->second->prezime,it->second->adresa,it->second->telefon};
            mapa_korisnika.insert(std::make_pair(it->first,pok));
        }
        try {
            for (auto it=b.mapa_knjiga.begin(); it!=b.mapa_knjiga.end(); it++) {
                Knjiga *pok=new Knjiga(it->second->DajNaslov(),it->second->DajAutora(),it->second->DajZanr(),it->second->DajGodinuIzdavanja(),it->second->DajKodKogaJe());
...
void Biblioteka::PrikaziZaduzenja(int clanski_broj) const {
    ...
    for (auto it2=mapa_knjiga.begin(); it2!=mapa_knjiga.end(); it2++) {
        if (it2->second->DaLiJeZaduzena() && it2->second->DajKodKogaJe()==it1->second) {
