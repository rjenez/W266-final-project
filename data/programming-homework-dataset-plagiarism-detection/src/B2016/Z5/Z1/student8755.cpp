...
class Biblioteka
{
    std::map<int, Korisnik*> m_korisnik;
    ...
    ~Biblioteka(){for(auto it=m_korisnik.begin(); it!=m_korisnik.end(); it++) delete it->second; for(auto it=m_knjiga.begin(); it!=m_knjiga.end(); it++) delete it->second;}
...
Biblioteka::Biblioteka(Biblioteka &bible)
{
    for(auto it=bible.m_korisnik.begin(); it!=bible.m_korisnik.end(); it++)
        m_korisnik.insert(std::make_pair(it->first, new Korisnik{it->second->ime, it->second->prezime, it->second->adresa, it->second->telefon}));
    for(auto it=bible.m_knjiga.begin(); it!=bible.m_knjiga.end(); it++)
        m_knjiga.insert(std::make_pair(it->first, new Knjiga{it->second->DajNaslov(), it->second->DajAutora(), it->second->DajZanr(), it->second->DajGodinuIzdavanja()}));
}
Biblioteka::Biblioteka(Biblioteka &&bible)
{
    for(auto it=bible.m_korisnik.begin(); it!=bible.m_korisnik.end(); it++)
...
void Biblioteka::RegistrirajNovogKorisnika(int cb, std::string ime, std::string prezime, std::string adresa, std::string tel)
{
    ...
    Korisnik *user(new Korisnik{ime, prezime, adresa, tel});
...
void Biblioteka::RegistrirajNovuKnjigu(int eb, std::string naslov, std::string pisac, std::string zanr, int g)
{
    ...
    Knjiga *book(new Knjiga(naslov, pisac, zanr,g));
