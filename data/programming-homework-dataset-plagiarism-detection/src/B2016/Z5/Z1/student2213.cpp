    ...
    public:
        ...
        Biblioteka(Biblioteka &b){
            ...
            for(auto val : b.mapa_korisnika){
                Korisnik *novi_korisnik(new Korisnik{val.second -> ime, val.second -> prezime, val.second -> adresa, val.second -> telefon});
                int clanski_kopija = val.first;
                
                mapa_korisnika.insert(pair<int, Korisnik*>(clanski_kopija, novi_korisnik));
            }
            
           for(auto val : b.mapa_knjiga){
               Knjiga *nova_knjiga(new Knjiga(val.second -> DajNaslov(), val.second -> DajAutora(), val.second -> DajZanr(), val.second -> DajGodinuIzdavanja()));
               int evidencijski_kopija = val.first;
               
               mapa_knjiga.insert(pair<int, Knjiga*>(evidencijski_kopija, nova_knjiga));
           }
        }
        ...
        void RegistrirajNovogKorisnika(int clanskiBroj, string _ime, string _prezime, string _adresa, string _telefon){
                ...
                for(auto val : mapa_korisnika){
                    ...
                    Korisnik* n_korisnik(new Korisnik{_ime, _prezime, _adresa, _telefon});
                    mapa_korisnika.insert(pair<int, Korisnik*> (clanskiBroj, n_korisnik));
        ...
        void RegistrirajNovuKnjigu(int evidencijskiBroj, string _naslov, string _ime_pisca, string _zanr, int _godina_izdavanja){
                ...
                Knjiga* n_knjiga(new Knjiga(_naslov, _ime_pisca, _zanr, _godina_izdavanja));
                mapa_knjiga.insert(pair<int, Knjiga*> (evidencijskiBroj, n_knjiga));
        ...
        void ZaduziKnjigu(int evid, int clans){
            ...
            else mapa_knjiga.find(evid) -> second -> ZaduziKnjigu((mapa_korisnika[clans]));
        }
        ...
        void PrikaziZaduzenja(int clans) const{
            Korisnik* korisnik_temp = mapa_korisnika.find(clans) -> second;
...
int main ()
{
    ...
    /*
    Biblioteka bib;
    bib.RegistrirajNovogKorisnika(1, "ja", "jap", "ad", "053");
    bib.RegistrirajNovuKnjigu(4,"naslov", "autor", "zanrs", 190);
    
    bib.ZaduziKnjigu(4,1);
    
    bib.IzlistajKorisnike();
    bib.IzlistajKnjige();
    
    cout << "PRIKAZUJEM ZADUZENJA" << endl;
