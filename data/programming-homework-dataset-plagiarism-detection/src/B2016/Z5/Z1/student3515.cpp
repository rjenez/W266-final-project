...
class Biblioteka {
   map<int, Korisnik*> mkorisnici;
   map<int, Knjiga*> mknjige;
   public:
   Biblioteka() {};
...
void Biblioteka::ZaduziKnjigu(int br_knj, int cl_br) {
    ...
    for(auto it=mkorisnici.begin();it!=mkorisnici.end();it++) {
        if(it->first==cl_br) {
            ...
            if(iter->first==br_knj)  iter->second->DajKodKogaJe()=it->second; }
        }
    }
}
...
void Biblioteka::PrikaziZaduzenja(int cl_br) {
    ...
    for(auto it=mkorisnici.begin();it!=mkorisnici.end();it++) {
        ...
        bool ima=false;
        for(auto iter=mknjige.begin();iter!=mknjige.end();iter++) {
            if(it->second==iter->second->DajKodKogaJe()) { ima=true;
