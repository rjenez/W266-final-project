...
public:
        ...
        for (auto it = ZaKopirati.korisnici.begin(); it != ZaKopirati.korisnici.end(); ++it)
            try {
                Biblioteka::korisnici.insert(std::make_pair(it->first,
                    new Korisnik {it->second->ime, it->second->prezime, it->second->adresa, it->second->telefon} ));
            }   catch (std::bad_alloc()) {
                for (auto it = Biblioteka::korisnici.begin(); it != Biblioteka::korisnici.end(); ++it)
                    delete it->second, it->second = nullptr;    Biblioteka::korisnici.clear(); throw std::bad_alloc();
            }
        
        std::map<int,int> Poveznice;
        for (auto it = ZaKopirati.knjige.begin(); it != ZaKopirati.knjige.end(); ++it) 
            try {
                Biblioteka::knjige.insert(std::make_pair(it->first, 
                    new Knjiga {it->second->DajNaslov(), it->second->DajAutora(), it->second->DajZanr(), it->second->DajGodinuIzdavanja()} ));
                if (it->second->DaLiJeZaduzena()) {
                    
                    for (auto temp_it = ZaKopirati.korisnici.begin(); temp_it != ZaKopirati.korisnici.end(); ++temp_it)
                        if (it->second->DajKodKogaJe() == temp_it->second) {
                            Poveznice.insert(std::make_pair(it->first, temp_it->first)); break;
                        }
                }
            }   catch (std::bad_alloc()) {
                for (auto it = Biblioteka::korisnici.begin(); it != Biblioteka::korisnici.end(); ++it)
                    delete it->second, it->second = nullptr;    Biblioteka::korisnici.clear();
                for (auto it = Biblioteka::knjige.begin(); it != Biblioteka::knjige.end(); ++it)
                    delete it->second, it->second = nullptr;    Biblioteka::knjige.clear(); throw std::bad_alloc();
            }
            
        for (auto it = Poveznice.begin(); it != Poveznice.end(); ++it) {
            try {
                Biblioteka::ZaduziKnjigu(it->first, it->second);
            }   catch (...) {}
        }
    }
    ...
    Biblioteka &operator =(const Biblioteka &ZaKopirati) {  // preklapanje kopirajuceg operatora dodjele ..
        if (&ZaKopirati != this) {
            for (auto it = Biblioteka::knjige.begin(); it != Biblioteka::knjige.end(); ++it)
                delete it->second, it->second = nullptr;    Biblioteka::knjige.clear();
            for (auto it = Biblioteka::korisnici.begin(); it != Biblioteka::korisnici.end(); ++it)
                delete it->second, it->second = nullptr;    Biblioteka::korisnici.clear();
    ...
    Biblioteka &operator =(Biblioteka &&ZaPomjeriti) {      // preklapanje pomjerajuceg opetarotra dodjele ..
        if (&ZaPomjeriti != this) {
            for (auto it = Biblioteka::knjige.begin(); it != Biblioteka::knjige.end(); ++it)
                delete it->second, it->second = nullptr;    knjige.clear();
            for (auto it = Biblioteka::korisnici.begin(); it != Biblioteka::korisnici.end(); ++it)
                delete it->second, it->second = nullptr;    korisnici.clear();
    ...
    void RegistrirajNovogKorisnika (int clanski_br, const std::string &im, const std::string &pr, const std::string &ad, const std::string br_tel) {
        ...
        try {   
            Biblioteka::korisnici.insert(std::make_pair(clanski_br, new Korisnik {im, pr, ad, br_tel}));
        }   catch (std::bad_alloc()) {
            for (auto it = Biblioteka::korisnici.begin(); it != Biblioteka::korisnici.end(); ++it)
                delete it->second, it->second = nullptr;    Biblioteka::korisnici.clear();  throw std::bad_alloc();
        }
    ...
    void RegistrirajNovuKnjigu (int evidencijski_br, const std::string &naslov, const std::string &ime, const std::string &zanr, int godina) {
        ...
        try {
            Biblioteka::knjige.insert(std::make_pair(evidencijski_br, new Knjiga {naslov, ime, zanr, godina}));
        }   catch (std::bad_alloc()) {
            for (auto it = Biblioteka::knjige.begin(); it != Biblioteka::knjige.end(); ++it)
                delete it->second, it->second = nullptr;    Biblioteka::knjige.clear();     throw std::bad_alloc();
        }
    ...
    void ZaduziKnjigu (int evidencijski_broj, int clanski_broj) {
        ...
        pok_knjiga->ZaduziKnjigu(*pok_korisnik); // Zaduzenje ..
    }
    ...
    void PrikaziZaduzenja (int clanski_broj) {
        ...
        for (auto it = Biblioteka::knjige.begin(); it != Biblioteka::knjige.end(); ++it) {
            if (it->second->DajKodKogaJe() == pok_korisnik) {   DaLiSeNeNalazi = false;
                std::cout << "Evidencijski broj: " << it->first << std::endl << "Naslov: " << it->second->DajNaslov() << std::endl;
                std::cout << "Pisac: " << it->second->DajAutora() << std::endl << "Zanr: " << it->second->DajZanr() << std::endl;
