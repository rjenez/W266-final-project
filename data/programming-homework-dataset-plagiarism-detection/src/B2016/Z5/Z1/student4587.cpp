...
public:
  ...
  Biblioteka(const Biblioteka &b) {
    ...
    for(auto &x : b.korisnici) {
      korisnici[x.first]=new Korisnik (*(x.second) ); //prepisujemo clanski broj i alociramo prostor novi za korisnika koji inicijaliziramo na odg vrijednosti;
    }
    for(auto &x : b.knjige) {
      knjige[x.first]=new Knjiga (*(x.second) ); //prepisujemo evidencijski broj i alociramo prostor novi za knjgu koji inicijaliziramo na odg vrijednosti; sa tim da moramo pok na zaduzenje premjestiti !
  ...
  Biblioteka &operator=(const Biblioteka &b) {
    if(this!=&b) {
      for(auto &x : b.korisnici) {
        delete korisnici[x.first];
        korisnici[x.first]=new Korisnik (*(x.second) ); //prepisujemo clanski broj i alociramo prostor novi za korisnika koji inicijaliziramo na odg vrijednosti;
      }
      for(auto &x : b.knjige) {
        delete knjige[x.first];
        knjige[x.first]=new Knjiga (*(x.second) ); //prepisujemo evidencijski broj i alociramo prostor novi za knjgu koji inicijaliziramo na odg vrijednosti; sa tim da moramo pok na zaduzenje premjestiti !
        (*(knjige[x.first])).RazduziKnjigu(); // zaduzenje postavljamo na nullptr
      }
    }
  ...
  void RegistrirajNovogKorisnika(int broj, std::string ime,std::string prezime,std::string adresa,std::string telefon) {
    ...
    korisnici[broj]=new Korisnik {ime,prezime,adresa,telefon};
  ...
  void RegistrirajNovuKnjigu(int broj, std::string naslov_knjige,std::string ime_pisca,std::string zanr,int godina_izdavanja) {
    ...
    knjige[broj]=new Knjiga {naslov_knjige,ime_pisca,zanr,godina_izdavanja};
  ...
  void IzlistajKorisnike()const {
    std::for_each(std::begin(korisnici),std::end(korisnici), [](std::pair<int, Korisnik*>x) {
  ...
  void IzlistajKnjige()const {
    std::for_each(std::begin(knjige),std::end(knjige), [](std::pair<int, Knjiga*>x) {
  ...
  void PrikaziZaduzenja (int clanski) {
    int brojac(0);
    for(auto x : knjige) {
      if( (*(x.second)).DajKodKogaJe() == &NadjiKorisnika(clanski) ) {
        std::cout <<"Evidencijski broj: "<<x.first << std::endl;
        std::cout << "Naslov: " <<(*(x.second)).DajNaslov()<< std::endl;
        std::cout << "Pisac: " <<(*(x.second)).DajAutora()<< std::endl;
        std::cout << "Zanr: " <<(*(x.second)).DajZanr()<< std::endl;
