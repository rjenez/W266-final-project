...
Biblioteka::Biblioteka(const Biblioteka &b){
    ...
    for(auto it=b.knjige.begin(); it!=b.knjige.end(); it++){
        Knjiga *pok(new Knjiga(it->second->DajNaslov(),it->second->DajAutora(),it->second->DajZanr(),it->second->DajGodinuIzdavanja()));
...
void Biblioteka::RegistrirajNovuKnjigu(int evidencijski_broj, std::string naslov, std::string autor, std::string zanr, int godina){
    ...
    Knjiga *pok(new Knjiga(naslov,autor,zanr,godina));
...
void Biblioteka::PrikaziZaduzenja(int clanski_broj) const{
    ...
    bool postoji2=false;
        for(auto it=knjige.begin(); it!=knjige.end(); it++){
            ...
            if(pom.ime == it->second->DajKodKogaJe()->ime && pom.prezime == it->second->DajKodKogaJe()->prezime &&
            pom.adresa == it->second->DajKodKogaJe()->adresa && pom.telefon == it->second->DajKodKogaJe()->telefon){ 
                postoji2=true;
                std::cout << "Evidencijski broj: " << it->first << std::endl;
                std::cout << "Naslov: " << it->second->DajNaslov() << std::endl;
                std::cout << "Pisac: " << it->second->DajAutora() << std::endl;
...
try {
 //std::cout<<"====TEST====";
 //AT17 (zamger) - testira metodu za prikaz zaduzenja
