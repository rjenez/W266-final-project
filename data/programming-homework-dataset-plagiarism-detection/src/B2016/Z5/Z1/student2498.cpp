...
public:
    Knjiga(std::string naslov, std::string ime_pisca, std::string zanr, int godina_izdavanja, Korisnik *korisnik) : naslov(naslov), ime_pisca(ime_pisca), zanr(zanr), godina_izdavanja(godina_izdavanja), korisnik(korisnik){
...
class Biblioteka {
    std::map<Korisnik, int> mapa_korisnika; // clanski broj korisnika - second
    std::map<Knjiga, int> mapa_knjiga;   // evidencijski broj knjige - second
public:
   /* Biblioteka();
    ~Biblioteka();
