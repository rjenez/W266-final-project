...
class Biblioteka {
    ...
    Biblioteka &operator = (const Biblioteka &bibl){
        ...
        for(auto it=korisnici.begin(); it!=korisnici.end(); it++){
            delete it->second; korisnici.erase(it);
        }
        for(auto it=knjige.begin(); it!=knjige.end(); it++){
            delete it->second; knjige.erase(it);
        } 
        for(auto it=bibl.korisnici.begin(); it!=bibl.korisnici.end(); it++){
            korisnici.insert(std::make_pair(it->first,new Korisnik (*it->second)));
        }
    ...
    void RegistrirajNovogKorisnika (int cl_br, std::string name, std::string surname, std::string address, std::string br_tel){
        ...
        try{
            ...
            pok->adresa=address;
            pok->telefon=br_tel;
            korisnici.insert (std::make_pair(cl_br,pok));
        }
        catch(...){
            delete pok;
        }
    ...
    void RegistrirajNovuKnjigu(int ev_br, std::string Naslov, std::string autor, std::string Zanr, int godina){
        ...
        Knjiga *pok=nullptr;
        try{
            pok=new Knjiga(Naslov,autor,Zanr,godina);
    ...
    void PrikaziZaduzenja(int cl_broj){
        ...
        Korisnik *pok=it_kor->second;
        for (auto it=knjige.begin(); it!=knjige.end(); it++){
            if (pok==it->second->DajKodKogaJe()){
