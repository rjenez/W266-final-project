...
class Biblioteka {
    std::map<int, Korisnik*> users;
    std::map<int, Knjiga*> books;
public:
    //Upravljački elementi
    Biblioteka() {};
    Biblioteka (const Biblioteka &biblioteka) {
        std::map<int, Korisnik*> kopija_users;
        auto it_korisnik((biblioteka.users).begin());
        while (it_korisnik != (biblioteka.users).end()) {
            kopija_users.insert(std::make_pair(it_korisnik->first, new Korisnik(*(it_korisnik->second))));
            it_korisnik++;
        } 
        std::map<int, Knjiga*> kopija_books;
        auto it_knjiga((biblioteka.books).begin());
        while (it_knjiga != (biblioteka.books).end()) {
            kopija_books.insert(std::make_pair(it_knjiga->first, new Knjiga(*(it_knjiga->second))));
            it_knjiga++;
        }
    ...
    ~Biblioteka() {
        auto it1(users.begin()); while (it1 != users.end()) delete it1->second, it1++;
        auto it2(books.begin()); while (it2 != books.end()) delete it2->second, it2++;
    }
    ...
    void RegistrirajNovogKorisnika (int clanski_broj_korisnika, std::string ime, std::string prezime, std::string adresa, std::string broj_telefona) {
        ...
        novi = new Korisnik{ime, prezime, adresa, broj_telefona};
    ...
    void RegistrirajNovuKnjigu (int evidencijski_broj_knjige, std::string naslov, std::string ime_pisca, std::string zanr, int godina_izdavanja) {
        ...
        Knjiga* nova(nullptr);
        nova = new Knjiga(naslov, ime_pisca, zanr, godina_izdavanja);
