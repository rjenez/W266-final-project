...
class Biblioteka {
    ...
    ~Biblioteka(){
        for(auto poc=mapa_korisnika.begin(); poc!=mapa_korisnika.end();poc++)
            delete poc->second;
        for(auto poc=mapa_knjiga.begin(); poc!=mapa_knjiga.end();poc++)
            delete poc->second;
    }
...
void Biblioteka::RegistrirajNovogKorisnika (int clanski_br, std::string ime, std::string prezime, std::string adresa, std::string telefon) {
    try{
        ...
        mapa_korisnika[clanski_br]=(new Korisnik{ime,prezime, adresa, telefon});
...
void Biblioteka::RegistrirajNovuKnjigu (int evid_br, std::string naslov, std::string ime_pisca, std::string zanr, int god_izdavanja){
    try{
        ...
        mapa_knjiga[evid_br]=(new Knjiga{naslov, ime_pisca, zanr, god_izdavanja});
    }catch(std::logic_error izuzetak){
        std::cout<<izuzetak.what()<<std::endl;
    }
}
