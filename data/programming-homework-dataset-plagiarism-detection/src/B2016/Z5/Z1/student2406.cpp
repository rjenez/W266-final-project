...
Biblioteka &operator=(Biblioteka &k){
    if(this!=&k){
        for(auto it(KORISNICI.begin());it!=KORISNICI.end();it++){
           std::map<int,Korisnik *>::iterator i;
        i=KORISNICI.find(it->first);
        KORISNICI.erase(i);
     }
     
     for(auto it(KNJIGE.begin());it!=KNJIGE.end();it++){
 std::map<int,Knjiga *>::iterator i;
    ...
    Korisnik *pok(nullptr);
     for(auto it(k.KORISNICI.begin());it!=k.KORISNICI.end();it++){
        ...
        for(auto it(k.KNJIGE.begin());it!=k.KNJIGE.end();it++){
            p=new Knjiga(it->second->naslov,it->second->pisac,it->second->zanr,it->second->g_izdavanja);
...
void RegistrirajNovuKnjigu(int evidencijski,std::string naslov,std::string pisac,std::string zanr,int g_izdavanja) {
        try {
            ...
            Knjiga *X(nullptr);
            X=new Knjiga(naslov,pisac,zanr,g_izdavanja);
...
std::cout<<"Evidencijski broj: "<<it->first<<std::endl
         ...
         std::cout<<"Zaduzena kod korisnika: "<<p->ime<<" "<<p->prezime;
         }
            std::cout<<std::endl;
        }
    }
