...
class Biblioteka{
    ...
    ~Biblioteka(){
        ...
        for(auto it=m1.begin(); it!=m1.end(); it++){
            delete (*it1).second; it1++;
        }
        for(auto it=m2.begin(); it!=m2.end(); it++){
            delete (*it2).second; it2++;
        }
        m1.clear(); m2.clear();
    }
    Biblioteka(const Biblioteka &b){
        ...
        for(auto it=b.m1.begin(); it!=b.m1.end(); it++){
            pointer=new Korisnik{(*it1).second->ime,(*it1).second->prezime,(*it1).second->adresa,(*it1).second->telefon};
        ...
        for(auto it=b.m2.begin(); it!=b.m2.end(); it++){
            point=new Knjiga((*it2).second->DajNaslov(),(*it2).second->DajAutora(),(*it2).second->DajZanr(),(*it2).second->DajGodinuIzdavanja());
    ...
    void RegistrirajNovuKnjigu(int broj, std::string naslov, std::string ime_pisca,  std::string zanr, int godina){
        ...
        if(m2.find(broj)!=m2.end()) throw std::logic_error("Knjiga vec postoji");
            Knjiga *k(nullptr);
            k=new Knjiga(naslov,ime_pisca,zanr, godina);
