...
class Biblioteka {
    ...
    /*Objekti tipa “Bibilioteka” moraju se moći kreirati ne navodeći nikakve dopunske informacije, a također se mogu bezbjedno
    kopirati i međusobno dodjeljivati, koristeći strategiju dubokog kopiranja, pri čemu su predviđene
    optimizirane verzije u slučaju kada se kopiraju privremeni objekti. Također, primjerci ove klase
    treba da se brinu o oslobađanju svih resursa koje su zauzeli tokom njihovog života. */
...
Biblioteka::~Biblioteka() {
    ...
    for ( ; it2!=mapa_knjiga.end(); it2++) {
        ...
        p=nullptr;
        delete it2->second; it2->second=nullptr;
    }
    mapa_knjiga.clear();
}
...
Biblioteka::Biblioteka (const Biblioteka &b) {
    ...
    for (; it!=b.mapa_korisnika.end(); it++) {
        ...
        kopija->ime=(it->second)->ime;
        kopija->prezime=(it->second)->prezime;
        kopija->adresa=(it->second)->adresa;
        kopija->telefon=(it->second)->telefon;
    ...
    for ( ; it2!=b.mapa_knjiga.end(); it2++) {
        Knjiga *kopija(new Knjiga(it2->second->DajNaslov(), it2->second->DajAutora(), it2->second->DajZanr(), it2->second->DajGodinuIzdavanja()));
...
void Biblioteka::RegistrirajNovogKorisnika(int cl_broj, std::string ime, std::string prezime, std::string adresa, std::string telefon) {
    Korisnik* pok_na_novog(new Korisnik);
    pok_na_novog->ime=ime; pok_na_novog->prezime=prezime; pok_na_novog->adresa=adresa; pok_na_novog->telefon=telefon;
...
void Biblioteka::RegistrirajNovuKnjigu(int ev_br, std::string naslov, std::string ime_pisca, std::string zanr, int godina_izdavanja) {
    auto pok_na_novu(new Knjiga(naslov, ime_pisca, zanr, godina_izdavanja));
