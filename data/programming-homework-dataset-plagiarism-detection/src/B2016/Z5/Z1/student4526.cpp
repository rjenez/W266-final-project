...
class Knjiga{ 
    ...
    Knjiga(string n,string i,string z, int g) : nasl(n),imep(i),zanr(z), godina(g), zaduzen(nullptr){}
...
class Biblioteka{
    std::map<int,Knjiga *> kn;
    std::map<int,Korisnik*>ko;
    public:
    Biblioteka(){}
    /*Biblioteka(const Biblioteka &a): kn->second(new Knjiga[a.kn.size()]),ko->second(new Korisnik[a.ko.size()]){
        kn.reize(a.kn.size());
        ko.reize(a.ko.size());
        auto it1(kn.begin());
        for(auto it(a.kn.begin();it!=a.kn.end();it++){
            it1->first=it->first;
            it1++;
        }
        std::copy(a.kn.second,a.kn.second+a.kn.size(),kn.second);
        
        std::copy(a.ko.second,a.ko.second+a.k0.size(),ko.second);
    ...
    void RegistrirajNovogKorisnika(int clbroj,string ime,string prezime,string adresa,string brojtel){
        ...
        ko.insert(std::make_pair(clbroj,new Korisnik(kor)));
    ...
    void RegistrirajNovuKnjigu(int evbroj,string naslov,string pisac,string zanr,int god){
        ...
        kn.insert(std::make_pair(evbroj,new Knjiga(naslov,pisac,zanr,god)));
    }
