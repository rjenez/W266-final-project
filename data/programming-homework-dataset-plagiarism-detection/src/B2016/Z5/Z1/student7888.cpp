...
#include <algorithm>
    ...
    class Knjiga {
      ...
      Korisnik *korisnici;
       ...
       Korisnik *DajKodKogaJe() const { return korisnici; }
       bool DaLiJeZaduzena() const { return korisnici; }
      void ZaduziKnjigu(Korisnik &korisnik1) { korisnici = &korisnik1; }
    ...
    class Biblioteka {
      ...
      void RegistrirajNovuKnjigu(int broj,std::string naslov1,std::string ime1,std::string zanr1, int godina1) {
        ...
        if (!knjige.count(broj)) {
         try {
          knjige[broj] = new Knjiga(naslov1,ime1,zanr1,godina1);
         }
        catch(...) {
          delete knjige[broj];
        }
