...
Biblioteka& Biblioteka::operator = (const Biblioteka &b) {
    std::map<int, std::shared_ptr<Korisnik>> tmp_korisnika;
    std::map<int, std::shared_ptr<Knjiga>> tmp_knjiga;
    try{
        for(auto it = b.m_korisnika.begin(); it != b.m_korisnika.end(); it++)
            tmp_korisnika[it->first] = std::make_shared<Korisnik>(Korisnik(*it->second));//new Korisnik (*it->second);
        for(auto it = b.m_knjiga.begin(); it != b.m_knjiga.end(); it++)
            tmp_knjiga[it->first] = std::make_shared<Knjiga>(Knjiga(*it->second));//new Knjiga (*it->second);
    }
    catch(std::bad_alloc) {
        throw;
    }
...
void Biblioteka::RegistrirajNovogKorisnika(int cl_br, std::string ime, std::string prezime, std::string adresa, std::string brtel) {
   if(m_korisnika.count(cl_br)) throw std::logic_error("Korisnik vec postoji");
    try{
        m_korisnika[cl_br] = std::make_shared<Korisnik>(Korisnik{ime, prezime, adresa, brtel});
    }
    catch(std::bad_alloc) {
        throw;
    }
...
void Biblioteka::RegistrirajNovuKnjigu(int ev_br, std::string naslov, std::string autor, std::string zanr, int god_izdavanja) {
    ...
    try{
        m_knjiga[ev_br] = std::make_shared<Knjiga>(Knjiga(naslov, autor, zanr, god_izdavanja));
    }
    catch(std::bad_alloc) {
        throw;
    }
