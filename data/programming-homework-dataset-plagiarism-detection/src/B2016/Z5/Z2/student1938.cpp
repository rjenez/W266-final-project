...
Biblioteka &Biblioteka::operator=(const Biblioteka &b){
    try{
        std::map<int,std::shared_ptr<Knjiga>> knjigee;
        for(auto &p:b.knjige)
            knjigee[p.first] = std::make_shared<Knjiga>(p.second->DajNaslov(),p.second->DajAutora(),
            p.second->DajZanr(),p.second->DajGodinuIzdavanja());
            
        std::map<int,std::shared_ptr<Korisnik>> clanovii;
        for(auto &p:b.clanovi){
            clanovii[p.first] = std::make_shared<Korisnik>();
    ...
    try{
        ...
        for(auto &p:knjige){
            if(p.second->DajKodKogaJe() == &k){
                std::cout << "Evidencijksi broj: " << p.first << std::endl <<
...
void Biblioteka::RegistrirajNovuKnjigu(int br, std::string naslov, std::string imeP,
    ...
    knjige[br] = std::make_shared<Knjiga>(naslov,imeP,zanr,god);
}
void Biblioteka::RegistrirajNovogKorisnika(int br,std::string ime,std::string prezime,
    ...
    clanovi[br] = std::make_shared< Korisnik>();
