...
class Biblioteka
{
    ...
    */
    void RegistrirajNovogKorisnika(int cl_broj, std::string ime, std::string prezime, std::string adresa, std::string br_telefona)
    {
        ...
        Korisnik novikorisnik{ime, prezime, adresa, br_telefona};
        std::shared_ptr<Korisnik> pametni_korisnik = std::make_shared<Korisnik>(novikorisnik);
        korisnik.insert(std::make_pair(cl_broj, pametni_korisnik));
    }
    ...
    void RegistrirajNovuKnjigu(int br_knjige, std::string naslov, std::string pisac, std::string zanr, int godina_izdavanja)
    {
        ...
        std::shared_ptr<Knjiga> novaknjiga = std::make_shared<Knjiga>(naslov, pisac, zanr, godina_izdavanja);
    ...
    {
        std::shared_ptr<Korisnik> k = nullptr;
        for(auto it = korisnik.begin(); it != korisnik.end(); it++)
        if(it->first == cl_broj)
            k = it->second;
