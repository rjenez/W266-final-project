...
class Knjiga {
    ...
    public : 
    Knjiga(std::string naslov, std::string ime, std::string zanr_knjige, int godina) : korisnik(nullptr) {
    ...
    std::shared_ptr<Korisnik> DajKodKogaJe() { return korisnik; }
    void ZaduziKnjigu(Korisnik &korisnik1) {
        korisnik = std::make_shared<Korisnik>(korisnik1);
    }
    void RazduziKnjigu() {
        korisnik = nullptr;
    }
...
class Biblioteka {
    ...
    Biblioteka (const Biblioteka &b) {
            ...
            for(auto it((b.mapa_korisnika).begin()); it != (b.mapa_korisnika).end(); it++) {
                ...
                korisnici.second = std::make_shared<Korisnik>(*(it->second));
                mapa_korisnika.insert(korisnici);
            }
            for(auto it((b.mapa_knjiga).begin()); it !=(b.mapa_knjiga.end()); it++) {
                mapa_knjiga.insert(std::make_pair(it->first, std::make_shared<Knjiga> (*(it->second))));
            }
    ...
    void RegistrirajNovogKorisnika (int clanski_broj_korisnika, std::string ime, std::string prezime, std::string adresa, std::string telefon) {
        ...
        mapa_korisnika[clanski_broj_korisnika] = std::make_shared<Korisnik>(novi);
    }
    ...
    void IzlistajKnjige() {
        for(auto it(mapa_knjiga.begin()); it != mapa_knjiga.end(); it++) {
           ...
           if((*(it -> second)).DaLiJeZaduzena()) {
               std::shared_ptr<Korisnik>vlasnik = (*(it->second)).DajKodKogaJe();
