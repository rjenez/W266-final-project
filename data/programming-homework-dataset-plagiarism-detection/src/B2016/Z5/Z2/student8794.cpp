...
public :
    ...
    void RegistrirajNovuKnjigu (int clanski_broj, std::string naslov,std::string ime_pisca, std::string zanr,int godina_izdavanja) {
        ...
        knjige.insert(std::make_pair(clanski_broj,std::make_shared<Knjiga>(Knjiga(naslov,ime_pisca,zanr,godina_izdavanja))));
    }
    ...
    void PrikaziZaduzenja (int broj_korisnika) {
        ...
        for (auto &i : knjige) {
            if(i.second->DajKodKogaJe() and i.second->DajKodKogaJe() == t->second.get()) {
                if(!flag) flag = true;
...
Biblioteka::Biblioteka (const Biblioteka &b)
{
    ...
    try  {
        ...
        for (auto i = knjige.begin(); i != knjige.end(); i++) {
            i->second = std::make_shared<Knjiga> (*(j->second));
            j++;
        }
        for (auto i = korisnici.begin(); i != korisnici.end(); i++) {
            i->second = std::make_shared<Korisnik>(*(k->second));
            k++;
        }
    } catch (...) {
