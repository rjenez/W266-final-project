...
public:
    Knjiga(std::string naslov,std::string autor,std::string zanr, int godina_izdavanja,std::shared_ptr<Korisnik> p=nullptr):naslov(naslov),autor(autor),zanr(zanr),godina_izdavanja(godina_izdavanja),zaduzenje(p) {}
    ~Knjiga() {}
    ...
    void ZaduziKnjigu(Korisnik& korisnik){
        zaduzenje=std::make_shared<Korisnik>(korisnik);
    }
    void RazduziKnjigu() {
        zaduzenje=nullptr;
    }
...
public:
    Biblioteka() {}
    Biblioteka(std::map<int, std::shared_ptr<Korisnik>>k1, std::map<int, std::shared_ptr<Knjiga>> k2):korisnici(k1),knjige(k2) {}
...
{
    std::map<int, std::shared_ptr<Korisnik>>novi_korisnici;
    std::map<int, std::shared_ptr<Knjiga>> nove_knjige;
    for(auto it=b.korisnici.begin(); it!=b.korisnici.end(); it++)
        novi_korisnici[it->first]=std::make_shared<Korisnik>(*(it->second));
...
void Biblioteka:: RegistrirajNovuKnjigu(int broj, std::string nasloov, std::string ime_pisca, std::string zanrr, int godinaizdavanja)
{
    if(knjige.count(broj)) throw std::logic_error("Knjiga vec postoji");
        ...
        std::shared_ptr<Knjiga> nova=std::make_shared<Knjiga>(nasloov,ime_pisca,zanrr,godinaizdavanja);
