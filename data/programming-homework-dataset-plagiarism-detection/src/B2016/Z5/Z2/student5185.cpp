...
class Knjiga{
        ...
        int godinaIzdavanja;
        std::shared_ptr<Korisnik> zaduzeno;
    public: //Konstruktor
    ...
    void ZaduziKnjigu(Korisnik &k){
        zaduzeno = std::make_shared<Korisnik>(k);
    }
    void RazduziKnjigu(){
        zaduzeno = nullptr;
    }
...
class Biblioteka{
        ...
        void Pomocna(int evBr, std::shared_ptr<Knjiga> knjiga) const{
    ...
    public:   
        ...
        Biblioteka(const Biblioteka &b){ 
            try{ //U slucaju nekog izuzetka
                for (auto i : b.korisnici){
                    std::shared_ptr<Korisnik>k (std::make_shared<Korisnik>(*i.second));
            ...
            try{
                for (auto i : b.knjige) {
                    std::shared_ptr<Knjiga> k(std::make_shared<Knjiga> (*i.second));
        ...
        Biblioteka(Biblioteka &&b){
            ...
            for (auto i : knjige) {i.second = nullptr;}
            for (auto i : korisnici) {i.second = nullptr;}
        ...
        void RegistrirajNovogKorisnika (int clanskiBroj, std::string ime, std::string prezime, std::string adresa, std::string telefon){
            ...
            std::shared_ptr<Korisnik>k1 (std::make_shared<Korisnik>());
        ...
        void RegistrirajNovuKnjigu(int evidencijskiBroj, std::string naslovKnjige, std::string imePisca, std::string zanr, int godinaIzdavanja){
            ...
            knjige.insert(std::make_pair(evidencijskiBroj, std::make_shared<Knjiga> (naslovKnjige, imePisca, zanr, godinaIzdavanja)));
        } //
        ...
        void PrikaziZaduzenja (int clanskiBroj) const{
            ...
            for (auto i : knjige) {
                std::shared_ptr<Korisnik> pokNaKorisnika(i.second->DajKodKogaJe());
