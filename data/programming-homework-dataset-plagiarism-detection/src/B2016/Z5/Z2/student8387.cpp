...
Biblioteka::Biblioteka(Biblioteka &b){
    for(auto i=b.korisnici.begin();i!=b.korisnici.end();i++){
           korisnici[i->first]=std::shared_ptr<Korisnik>(new Korisnik(*(i->second))); 
    }
    for(auto i=b.knjige.begin();i!=b.knjige.end();i++){
        knjige[i->first]=std::shared_ptr<Knjiga>(new Knjiga(*(i->second)));
    }
}
Biblioteka::Biblioteka(Biblioteka &&b){
    for(auto i=b.korisnici.begin();i!=b.korisnici.end();i++){
...
void Biblioteka::RegistrirajNovogKorisnika(int clbr,std::string ime,std::string prezime,std::string adresa,std::string broj){
    if(korisnici.count(clbr))throw std::logic_error("Korisnik već postoji");
    korisnici[clbr]=std::shared_ptr<Korisnik>(new Korisnik{ime,prezime,adresa,broj});
}
void Biblioteka::RegistrirajNovuKnjigu(int clbr,std::string naslov,std::string ime,std::string zanr,int godina){
    if(knjige.count(clbr))throw std::logic_error("Korisnik već postoji");
    knjige[clbr]=std::shared_ptr<Knjiga>(new Knjiga(naslov,ime,zanr,godina));
}
...
void Biblioteka::PrikaziZaduzenja(int clbr){
    if(!korisnici.count(clbr))throw std::domain_error("Korsinik nije nadjen");
    int br(0);
    for(auto i=knjige.begin();i!=knjige.end();i++){
