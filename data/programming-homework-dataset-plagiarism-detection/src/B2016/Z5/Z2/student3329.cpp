...
public:
    ...
    bool DaLiJeZaduzena()const{if(zaduzena==nullptr) return false; else return true;}
};

class Biblioteka{
    std::map<int, std::shared_ptr<Korisnik>> mkorisnika;
    std::map<int, std::shared_ptr<Knjiga>> mknjiga;
public:
    Biblioteka(){}
...
Biblioteka::Biblioteka(Biblioteka &b){
    for(auto it=b.mkorisnika.begin(); it!=b.mkorisnika.end(); it++){
        it->second=std::make_shared<Korisnik>();
        *mkorisnika[it->first]=*b.mkorisnika[it->first];
    }
    for(auto it=b.mknjiga.begin(); it!=b.mknjiga.end(); it++){
        auto knjiga(b.mknjiga[it->first]);
        mknjiga[it->first]=std::make_shared<Knjiga>(knjiga->DajNaslov(), knjiga->DajAutora(), knjiga->DajZanr(), knjiga->DajGodinuIzdavanja());
        //znam da je plitko
    }
}
...
Biblioteka &Biblioteka::operator=(Biblioteka &&b){
    ...
    for(auto it=b.mkorisnika.begin(); it!=b.mkorisnika.end(); it++) mkorisnika[it->first]=nullptr;
    for(auto it=b.mknjiga.begin(); it!=b.mknjiga.end(); it++) mknjiga[it->first]=nullptr; return *this;
}
void Biblioteka::RegistrirajNovogKorisnika(int cl_broj, std::string ime, std::string prezime, std::string adresa, std::string telefon){
    ...
    mkorisnika[cl_broj]=std::make_shared<Korisnik>();
...
void Biblioteka::RegistrirajNovuKnjigu(int ev_broj, std::string naslov, std::string ime_pisca, std::string zanr, int godina_izdavanja){
    ...
    mknjiga[ev_broj]=std::make_shared<Knjiga>(naslov, ime_pisca, zanr, godina_izdavanja);
}
...
void Biblioteka::PrikaziZaduzenja(int cl_broj){
    ...
    for(auto it=mknjiga.begin(); it!=mknjiga.end(); it++){
        if(mkorisnika[cl_broj]==std::shared_ptr<Korisnik>(mknjiga[it->first]->DajKodKogaJe())){
            std::cout<<"Evidencijski broj: "<<it->first<<std::endl;
            std::cout<<"Naslov: "<<mknjiga[it->first]->DajNaslov()<<std::endl;
            std::cout<<"Pisac: "<<mknjiga[it->first]->DajAutora()<<std::endl;
            std::cout<<"Zanr: "<<mknjiga[it->first]->DajZanr()<<std::endl;
