...
public:
    Biblioteka(){};
    ~Biblioteka(){ for(auto it=mapa_korisnika.begin(); it!=mapa_korisnika.end();++it){it->second=nullptr;}
...
void Biblioteka::RegistrirajNovogKorisnika(int clanski_broj, std::string ime, std::string prezime, std::string adresa, std::string telefon){
    ...
    Korisnik korisnik;
    auto pok(std::make_shared<Korisnik>(korisnik));
    pok->ime=ime;
    pok->prezime=prezime;
    pok->adresa=adresa;
    pok->telefon=telefon;
...
Biblioteka::Biblioteka(const Biblioteka &b){
    for(auto it=b.mapa_korisnika.begin(); it!=b.mapa_korisnika.end();it++){
        Korisnik korisnik;
        auto k(std::make_shared<Korisnik>(korisnik));
        k->ime=it->second->ime;
    ...
    for(auto it=b.mapa_knjiga.begin(); it!=b.mapa_knjiga.end();it++){
        auto knjiga(std::make_shared<Knjiga>(it->second->DajNaslov(), it->second->DajAutora(), it->second->DajZanr(), it->second->DajGodinuIzdavanja()));
        mapa_knjiga[it->first]=knjiga; 
    }  
}
...
Biblioteka &Biblioteka::operator=(const Biblioteka &b){
    if(&b != this){
        ...
        for(auto it=b.mapa_knjiga.begin(); it!=b.mapa_knjiga.end();++it){
            auto knjiga(std::make_shared<Knjiga>(it->second->DajNaslov(), it->second->DajAutora(), it->second->DajZanr(), it->second->DajGodinuIzdavanja()));
            mapa_knjiga.insert(std::make_pair(it->first,knjiga));
        }
        return *this;
    }
    return *this;
