...
class Knjiga {
    ...
    std::string zanr;
    int god_izdavanja;
    Korisnik *zaduzeno; //ovdje nije potreban pametni pokazivac!
public:
...
class Biblioteka {
    ...
    static void BezZaduzenja(std::shared_ptr<Knjiga> it) {
...
public:
    ...
    void RegistrirajNovogKorisnika(int clanski_br, std::string ime, std::string prezime, std::string adresa, std::string br_tel) {
        ...
        try {
            auto k(std::make_shared<Korisnik> (ime, prezime, adresa, br_tel));
    ...
    void RegistrirajNovuKnjigu(int ident_broj, std::string naslov, std::string autor, std::string zanr, int god) {
        ...
        try {
            auto k(std::make_shared<Knjiga> (naslov, autor, zanr, god));
