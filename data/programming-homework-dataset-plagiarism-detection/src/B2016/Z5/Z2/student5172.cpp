    ...
    Biblioteka (const Biblioteka &bib) {
        try {
            for(auto it=bib.korisnici.begin(); it!=bib.korisnici.end(); it++) {
                std::shared_ptr<Korisnik> pom(std::make_shared< Korisnik>(*it->second)); 
                korisnici.insert(std::make_pair(it->first, pom));
            }

            try {
                for(auto it=bib.knjige.begin(); it!=bib.knjige.end(); it++) {
                    std::shared_ptr<Knjiga> pom(std::make_shared< Knjiga>(*it->second));
                    knjige.insert(std::make_pair(it->first, pom));
                }
            } catch(std::bad_alloc) {
...
void RegistrirajNovuKnjigu(int clanskibroj, std::string naslov, std::string ime, std::string zanr, int godina){
    std::shared_ptr<Knjiga>pok= std::make_shared< Knjiga>(naslov, ime, zanr, godina); 
...
void PrikazZaduzenja (int clanskibroj) const{
    int ima=0;
    for(auto it=korisnici.begin(); it!=korisnici.end(); it++){
       if(it->first==clanskibroj){ima=1;}
    }
