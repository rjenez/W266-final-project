...
void Biblioteka::RegistrirajNovogKorisnika (int clanski_broj, std::string ime, std::string prezime, std::string adresa, std::string broj_telefona) {
    ...
    std::shared_ptr<Korisnik>pok(new Korisnik {});
    (*pok).ime = ime;
    (*pok).prezime = prezime;
    (*pok).adresa = adresa;
...
void Biblioteka::RegistrirajNovuKnjigu (int evi_broj, std::string naslov, std::string pisac, std::string zanr, int god_izdavanja) {
    ...
    std::shared_ptr<Knjiga> pok(std::make_shared<Knjiga> (naslov, pisac, zanr, god_izdavanja));
    knjige[evi_broj] = pok;
}
Korisnik &Biblioteka::NadjiKorisnika (int cl_broj) {
    std::shared_ptr<Korisnik>pomocni;
...
Biblioteka::Biblioteka (const Biblioteka &kop) {
    for (auto it=kop.korisnici.begin(); it!=kop.korisnici.end(); it++) {
        std::shared_ptr<Korisnik>pomocni (new Korisnik {});
    ...
    for (auto it=kop.knjige.begin(); it!=kop.knjige.end(); it++) {
        std::shared_ptr<Knjiga>pomocni(std::make_shared<Knjiga> (it->second->DajNaslov(), it->second->DajAutora(), it->second->DajZanr(), it->second->DajGodinuIzdavanja()));
...
{
    Biblioteka bib;
   /* Biblioteka proba, temp;
    proba.RegistrirajNovogKorisnika(12,"ime", "prezime", "adresa", "telefon");
    temp.RegistrirajNovogKorisnika(13,"drugo","prvo","cet","Peto");
    proba.RegistrirajNovuKnjigu (12, "naslov","zanr", "autoe", 123);
    temp.RegistrirajNovuKnjigu(16, "E", "A", "EEE", 122);
    proba = temp;
    proba.IzlistajKnjige();*/
