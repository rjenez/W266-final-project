...
class Biblioteka {
    ...
    std::map<int , std::shared_ptr<Korisnik>> mapa_knjiga;
    public:
    Biblioteka() {}
    /*~Biblioteka() {
...
Biblioteka::Biblioteka(const Biblioteka &b) {
    try{
        for(auto it = b.mapa_korisnik.begin(); it != b.mapa_korisnik.end(); it++) {
            mapa_korisnik.insert(std::make_pair(it->first, std::make_shared<Korisnik>(*it->second)));
        }
        try{
...
void Biblioteka::RegistrirajNovogKorisnika(int clanski_broj, std::string ime, std::string prezime, std::string adresa, std::string broj_mobitela) {
   ...
   /* Korisnik k;
    ...
    mapa_korisnik.insert(std::make_pair(clanski_broj, std::make_shared<Korisnik>({ime, prezime, adresa, broj_mobitela})));
}
...
void Biblioteka::RegistrirajNovuKnjigu(int broj_knjige, std::string naslov, std::string ime_pisca, std::string zanr, int godina){
    ...
    mapa_knjiga.insert(std::make_pair(broj_knjige, std::make_shared<Knjiga>(naslov, ime_pisca, zanr, godina)));
}
