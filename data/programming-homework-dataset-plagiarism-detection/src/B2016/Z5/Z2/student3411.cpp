...
Biblioteka &Biblioteka::operator =(Biblioteka &&bibl){
    if(&bibl!=this){
        for(auto it=korisnik.begin(); it!=korisnik.end(); it++) it->second=nullptr;
        for(auto it=knjiga.begin(); it!=knjiga.end(); it++) it->second=nullptr;
    ...
    for(auto it=bibl.knjiga.begin(); it!=bibl.knjiga.end(); it++){
        ...
        auto pok(((it->second).get())->DajKodKogaJe());
        pok=nullptr;
        it->second=nullptr;
    }
    return *this;
...
Biblioteka &Biblioteka::operator=(const Biblioteka &bibl){
    if(&bibl!=this){
        for(auto it=korisnik.begin(); it!=korisnik.end(); it++) it->second=nullptr;
        for(auto it=knjiga.begin(); it!=knjiga.end(); it++) it->second=nullptr;    //u ova 4 reda se vrsi dealociranje
...
Biblioteka::Biblioteka(const Biblioteka &bibl){
    ...
    for(auto it1=bibl.korisnik.begin(); it1!=bibl.korisnik.end(); it1++){
        for(auto it=bibl.knjiga.begin(); it!=bibl.knjiga.end(); it++){
            auto osoba((it->second)->DajKodKogaJe());
            if(osoba==it1->second.get()){                             //zaduzivanje knjige da isti korisnik ima zaduzenu istu knjigu
                this->ZaduziKnjigu(it->first,it1->first);
            }
        }
    }
...
void Biblioteka::RegistrirajNovuKnjigu(int clanski,std::string naslov,std::string pisac,std::string zanr,int godina ){
    ...
    auto pok(std::make_shared<Knjiga>(naslov, pisac, zanr, godina));
