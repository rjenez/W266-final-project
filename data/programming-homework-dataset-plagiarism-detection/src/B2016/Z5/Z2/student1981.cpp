...
class Biblioteka{
    ...
    static void ProvjeraKorisnika(map<int,shared_ptr<Korisnik>> k,int n){
        for(auto it=k.begin();it!=k.end();it++)if(it->first==n)throw logic_error("Korisnik vec postoji");
    }
    static void ProvjeraKnjiga(map<int,shared_ptr<Knjiga>> k,int n){
   ...
   Biblioteka(){
    ...
    void RegistrirajNovogKorisnika(int clanski_broj,string ime,string prezime,string adresa,string broj_telefona){
        Korisnik korisnik{ime,prezime,adresa,broj_telefona};
        shared_ptr<Korisnik> k(make_shared<Korisnik>(korisnik));
    ...
    void RegistrirajNovuKnjigu(int evidencijski_broj,string naslov,string ime_pisca,string zanr,int godina_izdavanja){
        Knjiga knjiga{naslov,ime_pisca,zanr,godina_izdavanja};
        shared_ptr<Knjiga> k(make_shared<Knjiga>(knjiga));
