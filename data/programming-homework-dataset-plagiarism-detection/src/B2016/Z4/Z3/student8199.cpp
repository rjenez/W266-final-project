...
public:
    ...
    Prodaje (initializer_list<Prodaja> spisak_prodaja) {
        ...
        for (auto i: spisak_prodaja) p[stanje++]=new Prodaja(i);
    }
    Prodaje(const Prodaje &prodaje) {
        stanje=prodaje.stanje;
        kapacitet=prodaje.kapacitet;
        p=new Prodaja *[kapacitet];
        for (int i=0; i < stanje; i++) p[i]=new Prodaja(*prodaje.p[i]);
    }
    ...
    ~Prodaje() {
        for (int i=0; i < stanje; i++) delete p[i];
        delete [] p;
    }
    ...
    void RegistrirajProdaju(const string &ime_agenta_prodaje, double cijena_stana, const Datum &datum_prodaje, const Kupac &kupac_stana, const Stan &kupljeni_stan) {
        p[stanje++]=new Prodaja(ime_agenta_prodaje, cijena_stana, datum_prodaje, kupac_stana, kupljeni_stan);
    }
    void RegistrirajProdaju(const string &ime_agenta_prodaje, double cijena_stana, int dan_prodaje, int mjesec_prodaje, int godina_prodaje, string &ime_kupca, const Datum &datum_rodjenja_kupca, const string &adresa_stana, int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata) {
        p[stanje++]=new Prodaja(ime_agenta_prodaje, cijena_stana, dan_prodaje, mjesec_prodaje, godina_prodaje, ime_kupca, datum_rodjenja_kupca, adresa_stana, sprat_stana, broj_soba, namjesten_stan, broj_kvadrata);
    }
    void RegistrirajProdaju(Prodaja *prodaja) {
        p[stanje++]=new Prodaja(*prodaja);
    }
    ...
    int DajBrojProdajaNaDatum(const Datum &datum) const {
        ...
        for (int i=0; i < stanje; i++) {
            if (p[i]->DajDatumProdaje().DajDan()==datum.DajDan() && p[i]->DajDatumProdaje().DajMjesec()==datum.DajMjesec() && p[i]->DajDatumProdaje().DajGodinu()==datum.DajGodinu()) br++;
        }
        return br;
    }
    int DajBrojProdajaOdAgenta(const string &ime_agenta) const {
        int br=0;
        for (int i=0; i < stanje; i++) {
            if (p[i]->DajImeAgenta()==ime_agenta) br++;
        }
        return br;
    }
    Prodaja &DajNajranijuProdaju () {
        int mi=0;
        for (int i=0; i < stanje; i++) {
            if (PorediDatume(*p[i], *p[mi])==-1) mi=i;
        }
        return *p[mi];
    }
    ...
    Prodaja &DajNajskupljuProdaju () {
        int mi=0;
        for (int i=0; i < stanje; i++) {
            if (p[mi]->DajCijenuStana() < p[i]->DajCijenuStana()) mi=i;
        }
        return *p[mi];
    }
    ...
    void ObrisiNajranijuProdaju () {
        ...
        delete p[mi];
        for (int i=mi; i < stanje - 1; i++) p[i]=p[i + 1];
        stanje--;
    }
    void ObrisiProdajeAgenta(const string &ime_agenta) {
        for(int i=0; i < stanje; i++) {
            if (p[i]->DajImeAgenta()==ime_agenta) {
                delete p[i];
                stanje--;
                for (int j=i; j < stanje; j++) p[j]=p[j + 1];
            }
        }
    }
    void ObrisiProdajeNaDatum(const Datum &datum) {
        for (int i=0; i < stanje; i++) {
            int dat1=datum.DajDan() + datum.DajMjesec() * 100 + datum.DajGodinu() * 10000;
            int dat2=p[i]->DajDatumProdaje().DajDan() + p[i]->DajDatumProdaje().DajMjesec() * 100 + p[i]->DajDatumProdaje().DajGodinu() * 1000;
            if (dat1==dat2) {
                delete p[i];
                stanje--;
                for (int j=i; j < stanje; j++) p[j]=p[j + 1];
    ...
    void IspisiProdajeNaDatum(const Datum &datum) {
        for (int i=0; i < stanje; i++) p[i]->Ispisi();
    }
};
