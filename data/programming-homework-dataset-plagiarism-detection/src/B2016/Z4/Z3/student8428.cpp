...
class Prodaje
{
    ...
    void Dealociraj();
    public:
    explicit Prodaje(int max_broj_prodaja): MaxBrojProdaja(max_broj_prodaja)
    {
        Alociraj();
    }
    ...
    int DajBrojProdajaNaDatum(const Datum &datum) const
    {
        ...
        brojProdajaNaDatum = std::count_if(pok, pok + BrojProdaja, [&](Prodaja *p){
    ...
    int DajBrojProdajaOdAgenta(const std::string &ime_agenta) const
    {
        ...
        brojProdajaAgenta = std::count_if(pok, pok + BrojProdaja, [&](Prodaja *p){
    ...
    {
        return **std::min_element(pok, pok + BrojProdaja, [](Prodaja* p1, Prodaja* p2){
    ...
    {
        return (**std::max_element(pok, pok + BrojProdaja, [](Prodaja* p1, Prodaja* p2){
            return p1->DajCijenuStana() < p2->DajCijenuStana();
        }));
    }
    ...
    void ObrisiNajranijuProdaju()
    {
        ...
        std::remove(pok, pok + BrojProdaja, &prva);
        delete &prva;
        BrojProdaja--;
    }
    ...
    {
        std::for_each(pok, pok + BrojProdaja, [&](Prodaja* p)
        {
...
void Prodaje::Alociraj()
{
    try
    {
        pok = new Prodaja*[MaxBrojProdaja];
...
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja): MaxBrojProdaja(spisak_prodaja.size())
{
    try
    {
        Alociraj();
...
const Stan &kupljeni_stan)
{
    ...
    pok[BrojProdaja] = new Prodaja(ime_agenta_prodaje, cijena_stana, datum_prodaje, kupac_stana, kupljeni_stan);
    BrojProdaja++;
}
 ...
 int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata)
{
    if(BrojProdaja == MaxBrojProdaja)
        throw std::range_error("Dostignut maksimalni broj prodaja");
    pok[BrojProdaja] = new Prodaja(ime_agenta_prodaje, cijena_stana, dan_prodaje, mjesec_prodaje, godina_prodaje, 
...
Prodaje::Prodaje(const Prodaje &prodaje): BrojProdaja(prodaje.BrojProdaja), MaxBrojProdaja(prodaje.MaxBrojProdaja)
{
    try
    {
        Alociraj();
...
Prodaje::Prodaje(Prodaje &&prodaje): BrojProdaja(prodaje.BrojProdaja), MaxBrojProdaja(prodaje.MaxBrojProdaja), pok(nullptr)
{
    pok = prodaje.pok;
    prodaje.pok = nullptr;
}
Prodaje &Prodaje::operator =(const Prodaje &prodaje)
{
    ...
    for(int i(0); i < BrojProdaja; i++)
        delete pok[i];
    if(MaxBrojProdaja != prodaje.MaxBrojProdaja)
    {
        delete [] pok;
        MaxBrojProdaja = prodaje.MaxBrojProdaja;
        Alociraj();
    }
...
void Prodaje::ObrisiProdajeAgenta(const std::string &ime_agenta)
{
    ...
    for(int i(0); i < BrojProdaja; i++)
    {
        if(pok[i]->DajImeAgenta() == ime_agenta)
        {
            delete pok[i];
            brojProdaja++;
            pok[i] = pok[BrojProdaja-brojProdaja];
        }
    }
