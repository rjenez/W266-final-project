...
class Stan {
    ...
    void Ispisi() const { 
        ...
        if (DajNamjesten()) cout <<"je namjesten." << endl;
        else cout << "nije namjesten." << endl;
    }
};
...
void Prodaja::Ispisi() const {
    cout << std::setw(32) << "Ime agenta: " << ime_agenta << endl;
    cout << std::setw(32) << "Ime kupca: ";  kupac.Ispisi(); cout << endl;
    cout << std::setw(32) << "Zakazani datum prodaje: "; dat_pr.Ispisi() ; cout << endl;
    cout << std::setw(32) << "Cijena stana: " << cijena << endl;
    cout << std::setw(32) << "Informacije o stanu: " << endl; stan.Ispisi(); cout << endl;
}
...
class Prodaje {
    ...
    public:
    explicit Prodaje(int max_broj_prodaja) : broj_prodaja(0), kapacitet(max_broj_prodaja), prodaje_pok(new Prodaja*[max_broj_prodaja] {}) {}
...
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja) : broj_prodaja(spisak_prodaja.size()), kapacitet(spisak_prodaja.size()), prodaje_pok(new Prodaja*[spisak_prodaja.size()] {} ) {
    std::initializer_list<Prodaja>::iterator it{spisak_prodaja.begin()};
	try {
		int i(0);
		while (it!=spisak_prodaja.end()) {
			prodaje_pok[i]=new Prodaja(*it);
			it++; i++;
		}
	}
	catch (...) {
...
Prodaje::~Prodaje() {
    for (int i=0; i<kapacitet; i++) delete prodaje_pok[i];
    delete[] prodaje_pok;
}
Prodaje::Prodaje(const Prodaje &prodaje) :broj_prodaja(prodaje.broj_prodaja), kapacitet(prodaje.kapacitet), prodaje_pok(new Prodaja*[prodaje.kapacitet] {}) {
    //if (kapacitet!=prodaje.kapacitet) throw std::logic_error ("Nesaglasni kapaciteti"); 
	try {
		for (int i=0; i<prodaje.broj_prodaja; i++) {
			prodaje_pok[i]= new Prodaja(*prodaje.prodaje_pok[i]);
		}
		for (int i=prodaje.broj_prodaja; i<prodaje.kapacitet; i++) prodaje_pok[i]=nullptr; //?
	}
	catch (...) {
		for (int i=0; i<prodaje.kapacitet; i++) {
			delete prodaje_pok[i];
		}
		delete[] prodaje_pok; throw;
	}
}
Prodaje &Prodaje::operator =(const Prodaje &prodaje) {
    ...
    if (prodaje.broj_prodaja>broj_prodaja) {
        try {
            //Potrebno alocirati jos mjesta
			for (int i= broj_prodaja; i< prodaje.broj_prodaja; i++)
			prodaje_pok[i]= new Prodaja(*prodaje.prodaje_pok[i]);
		}
		catch (...) {
			for (int i= broj_prodaja; i< prodaje.broj_prodaja; i++) {
				delete prodaje_pok[i]; prodaje_pok[i]=nullptr;
			}
			throw;
		}
    }
    else {
        //Potrebno dealocirati visak
	...
	for (int i=0; i<broj_prodaja; i++) {
	*prodaje_pok[i]=*prodaje.prodaje_pok[i];
	}
	return *this;
}
Prodaje::Prodaje(Prodaje &&prodaje) : broj_prodaja(prodaje.broj_prodaja), kapacitet(prodaje.kapacitet), prodaje_pok(prodaje.prodaje_pok) {
    prodaje.prodaje_pok=nullptr; prodaje.broj_prodaja=0;
}
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, const Datum &datum_prodaje, const Kupac &kupac_stana,
    const Stan &kupljeni_stan) {
        ...
        auto pnp(new Prodaja(ime_agenta_prodaje, cijena_stana, datum_prodaje, kupac_stana, kupljeni_stan));
        if (broj_prodaja==kapacitet) throw std::range_error("Dostignut maksimalni broj prodaja");
        else {
            prodaje_pok[broj_prodaja++]=pnp;
        }
    }
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, int dan_prodaje,
    ...
    const Datum &datum_rodjenja_kupca, const std::string &adresa_stana,
    int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata) {
        auto pnp(new Prodaja(ime_agenta_prodaje, cijena_stana, dan_prodaje, mjesec_prodaje, godina_prodaje, ime_kupca, datum_rodjenja_kupca, adresa_stana,
...
int Prodaje::DajBrojProdajaNaDatum(const Datum &datum) const {
    ...
    return std::count_if(prodaje_pok, prodaje_pok+broj_prodaja, [datum] (Prodaja *p) -> bool {
...
int Prodaje::DajBrojProdajaOdAgenta(const std::string &ime_agenta) const {
    //if (broj_prodaja==0) throw std::domain_error ("Nema registriranih prodaja");
    return std::count_if(prodaje_pok, prodaje_pok+broj_prodaja, [ime_agenta] (Prodaja *p) -> bool {
...
Prodaja &Prodaje::DajNajranijuProdaju() { //Pazi gdje stavljas &!
    if (broj_prodaja==0) throw std::domain_error ("Nema registriranih prodaja");
    return **(std::min_element(prodaje_pok, prodaje_pok+broj_prodaja, [] (Prodaja *p1, Prodaja *p2) -> bool {
...
Prodaja Prodaje::DajNajranijuProdaju() const {
    if (broj_prodaja==0) throw std::domain_error ("Nema registriranih prodaja");
    auto p(std::min_element(prodaje_pok, prodaje_pok+broj_prodaja, [] (Prodaja *p1, Prodaja *p2) -> bool {
        auto dat1(p1->DajDatumProdaje()), dat2(p2->DajDatumProdaje());
        if (dat1.DajGodinu()==dat2.DajGodinu()) {
            if (dat1.DajMjesec()==dat2.DajMjesec()) {
                return dat1.DajDan()<dat2.DajDan();
...
Prodaja &Prodaje::DajNajskupljuProdaju() {
    if (broj_prodaja==0) throw std::domain_error ("Nema registriranih prodaja");
    auto p(std::max_element(prodaje_pok, prodaje_pok+broj_prodaja, [] (Prodaja *p1, Prodaja *p2) -> bool {
...
void Prodaje::IspisiProdajeNaDatum(const Datum &datum) const {
    ...
    Prodaja **pomocni(new Prodaja*[broj_prodaja_na_datum] {});
    for (int i=0; i<broj_prodaja_na_datum; i++) {
        if (JednakDatum(prodaje_pok[i]->DajDatumProdaje(), datum))
        pomocni[i]=new Prodaja(*(prodaje_pok[i]));
    }
    std::sort(pomocni, pomocni+broj_prodaja, [] (Prodaja *p1, Prodaja *p2) -> bool {
...
void Prodaje::IspisiSveProdaje() const { //Ne smij promijeniti sadrzaj objekta nad kojim se poziva
    //Sortirajmo
    Prodaja **pomocni(new Prodaja*[broj_prodaja] {});
    for (int i=0; i<broj_prodaja; i++) {
        pomocni[i]=new Prodaja(*(prodaje_pok[i]));
    }
    std::sort(pomocni, pomocni+broj_prodaja, [] (Prodaja *p1, Prodaja *p2) -> bool {
            ...
            do {
                ...
                if (opcija_p==2) {
                    bool unos(false);
                        do {
                            try {
                                cout << "Odaberite prodaju koju zelite urediti. Dostupne prodaje su:"; 
                                for (int i=0; i<v_prodaja.size(); i++) {
                                    cout << i+1 << "." << endl;
                                    v_prodaja[i].Ispisi();
                                }
                                int pr(0);
                                cin >> pr;
                                cout << "Izaberite opciju:" << endl;
                                cout << " 0 - nazad" << endl << " 1 - izmjena agenta" << endl << " 2 - izmjena datuma prodaje" << endl <<
                                " 3 - izmjena kupca" << endl << " 4 - izmjena stana" << endl << " 5 - izmjena cijene stana" << endl; 
                                int opcija(0);
                                cin >> opcija;
                                if (opcija==0) break;
                                if (opcija==1) {
