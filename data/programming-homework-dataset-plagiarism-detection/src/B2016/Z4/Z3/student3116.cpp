...
class Prodaje
{
    ...
    void RegistrirajProdaju(const std::string &ime_agenta_prodaje,double cijena_stana, int dan_prodaje,int mjesec_prodaje, int godina_prodaje, std::string &ime_kupca,
     ...
     {
         pok[stanje++]=new Prodaja(ime_agenta_prodaje,cijena_stana,dan_prodaje,mjesec_prodaje,godina_prodaje,ime_kupca,datum_rodjenja_kupca,adresa_stana,sprat_stana,broj_soba,namjesten_stan,broj_kvadrata);
     }
    ...
    int DajBrojProdajaNaDatum(const Datum &datum) const
    {
        ...
        br=count_if(pok,pok+stanje,[&datum](Prodaja *p1){if(p1->DajDatumProdaje().DajDan()==datum.DajDan() && p1->DajDatumProdaje().DajMjesec()==datum.DajMjesec()
