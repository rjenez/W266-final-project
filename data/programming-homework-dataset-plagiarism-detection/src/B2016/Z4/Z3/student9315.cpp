...
class Prodaje{
    ...
    int max, cur;
    public:
        explicit Prodaje(int max_broj_prodaja):max(max_broj_prodaja),cur(0){
...
void Prodaje::ObrisiProdajeNaDatum(const Datum &datum){
    cur = (p+cur) - std::remove_if(p,p+cur,[datum,this](Prodaja *a){
       return istiDat(a->DajDatumProdaje(),datum); 
    });
}
void Prodaje::ObrisiProdajeAgenta(const std::string &ime_agenta){
    cur = (p + cur) - std::remove_if(p,p+cur,[ime_agenta](Prodaja *a){
        return ime_agenta == a->DajImeAgenta();
    });
}
void Prodaje::ObrisiNajranijuProdaju(){
    if(cur == 0) throw::std::range_error("Prazna kolekcija");
    std::remove_if(p,p+cur,[this](Prodaja *a){
       return a == &DajNajranijuProdaju(); 
    });
    cur--;
}
...
Prodaja Prodaje::DajNajskupljuProdaju()const{
    if(cur == 0) throw std::domain_error("Nema registriranih prodaja");
    return **std::max_element(p,p+cur,[](Prodaja *a, Prodaja *b){
       return SkupljiStan(*a,*b); 
    });
}
Prodaja &Prodaje::DajNajskupljuProdaju(){
    if(cur == 0) throw std::domain_error("Nema registriranih prodaja");
     Prodaja &x = **std::max_element(p,p+cur,[](Prodaja *a, Prodaja *b){
       return SkupljiStan(*a,*b); 
    });
    return x;
} 
Prodaja &Prodaje::DajNajranijuProdaju(){
    if(cur == 0) throw std::domain_error("Nema registriranih prodaja");
    Prodaja &x = **std::min_element(p,p+cur,[](Prodaja *a, Prodaja *b){
       return  ProdatPrije(*a,*b);
    });
    return x;
}
Prodaja Prodaje::DajNajranijuProdaju()const{
    if(cur == 0) throw std::domain_error("Nema registriranih prodaja");
    return **std::min_element(p,p+cur,[](Prodaja *a, Prodaja *b){
       return ProdatPrije(*a,*b); 
    });
}
...
int Prodaje::DajBrojProdajaNaDatum(const Datum &datum)const{
    return std::count_if(p,p+cur,[datum](Prodaja *asd){
...
void Prodaje::RegistritrajProdaju(const std::string &ime_agenta_prodaje,
    ...
    const Stan &kupljeni_stan){
        if(cur == max) throw std::range_error("Dostignut maksimalan broj prodaja");
    auto novi = new Prodaja(ime_agenta_prodaje, cijena_stana, datum_prodaje, kupac_stana, kupljeni_stan);
    p[cur] = novi; cur++;
}
Prodaje &Prodaje::operator=(Prodaje &&prodaje){
    if(&prodaje != this){
        for(int i = 0; i < max; i++) delete p[i]; delete[] p;
        p = prodaje.p; prodaje.p = nullptr;
        max = prodaje.max;
    }
    return *this;
}
Prodaje &Prodaje::operator=(const Prodaje &prodaje){
    if(&prodaje != this){
        ...
        try{
            ...
            for(int i = 0; i < prodaje.cur; i++) novi[i] = new Prodaja(*prodaje.p[i]);
            for(int i = 0; i < max; i++) delete p[i]; delete[] p;
...
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja):max(spisak_prodaja.size()){
    p = nullptr; p= new Prodaja*[spisak_prodaja.size()];
    int n(0);
    try{
        for(auto &i:spisak_prodaja){p[n] = new Prodaja(i); n++;}
    }catch(...){
...
void uredjivanjeKupaca(std::vector<Kupac> &kupci){
    ...
    for(;;){
        ...
        switch(n){
            case 0: return;
            case 1: {
                kupci.push_back(dodajKupcaX());
                std::cout << "Kupac je uspjesno dodan! \n";
            }break;
            case 2: {
                std::cout << "Odaberite kupca kojeg zelite urediti. "<<
                "Dostupni kupci su:\n";
                for(int i = 0; i < kupci.size(); i++){
                    std::cout << " "<< i+1<<" - "; kupci[i].Ispisi();
                    std::cout << std::endl;
                }
                int koji; std::cin >> koji;
                auto tmp (dodajKupcaX());
                kupci[koji-1].Postavi(tmp.DajImePrezime(),tmp.DajDatumRodjenja());
                std::cout << "Kupac je uspjesno izmijenjen!\n";
            }break;
            case 3:{
                for(auto &i:kupci) {i.Ispisi(); std::cout << std::endl;}
            }break;
            case 4:{
                throw std::runtime_error("ma cao");
            }break;
        }
    }
}
Kupac dodajKupcaX(){
    for(;;){
        try{
            ...
            int d,m,g; std::scanf("%d/%d/%d",&d,&m,&g);
            return Kupac(ime,Datum(d,m,g));
        }catch(std::domain_error err){
            std::cout << "Izuzetak: " << err.what() << std::endl;
        }
    }
}
void uredjivanjeProdaja(std::vector<Prodaja> &prodaje){
...
int main () {
    ...
    for(;;){
        ...
        try{
            switch(n){
                case 0: std::cout << "\nDovidjenja!"; return 0;
                case 1: uredjivanjeStanova(stanovi); break;
                case 2: return 0; uredjivanjeKupaca(kupci); break;
                case 3: return 0; uredjivanjeProdaja(prodaje); break;
            }
...
void uredjivanjeStanova(std::vector<Stan> &stanovi){
    void urediStanX(std::vector<Stan> &stanovi, int kojiStan);
    for(;;){
        std::cout << "Izaberite opciju:\n 0 - nazad\n 1 - dodavanje stana\n "<<
        ...
        switch(n2){
            ...
            case 1:{
                for(;;){
                    try{
                        std::cout << "Unesite adresu: "; std::cin.ignore(100,'\n');
                        std::string s; std::getline(std::cin,s);
                        std::cout << "Unesite sprat: ";
                        int sprat; std::cin >> sprat;
                        std::cout << "Unesite broj soba: ";
                        int soba; std::cin >> soba;
                        std::cout << "Da li je stan namjesten (1 - da, 0 - ne): ";
                        int namj; std::cin >> namj;
                        std::cout << "Unesite kvadraturu: ";
                        double kvad; std::cin >> kvad;
                        Stan stan(s,sprat,soba,namj,kvad);if(namj < 0 || namj > 1) throw std::domain_error("Neispravan unos podataka");
                        stanovi.push_back(stan);
                        std::cout << "Stan je uspjesno dodan!\n"; break;
                    }catch(std::domain_error err){
                        std::cout << "Izuzetak: " << err.what() <<". Molimo unesite opet.\n";
                        continue;
                    }
                }
            }break;
            ...
            case 4: throw std::runtime_error("mah");
        }
    }
}
void urediStanX(std::vector<Stan> &s,int kojiStan){
    for(;;){
        std::cout << "Odaberite polje koje zelite urediti:\n" <<
        " 0 - nazad\n 1 - adresa\n 2 - sprat\n 3 - broj soba\n" <<
        " 4 - da li je stan namjeste\n 5 - kvadratura\n 6 sve";
        int n(kojiStan),m; std::cin >> m; std::cin.ignore(100,'\n'); n--;
        switch(m){
            case 0: return;
            case 1: {
                for(;;){
                    try{
                        ...
                        s[n].Postavi(adresa,s[n].DajSprat(),s[n].DajBrojSoba(), s[n].DajNamjesten(),s[n].DajKvadraturu()); break;
                    }catch(std::domain_error err){
                        std::cout << "Izuzetak: " << err.what() << ". Molimo unesite opet." << std::endl;
                    }
                }
            }break;
            case 2: {
                for(;;){
                    try{
                        int sprat; std::cin >> sprat;
                        s[n].Postavi(s[n].DajAdresu(), sprat, s[n].DajBrojSoba(),s[n].DajNamjesten(),s[n].DajKvadraturu()); break;
                    }catch(std::domain_error err){
                        std::cout << "Izuzetak: " << err.what() << ". Molimo unesite opet." << std::endl;
                    }
                }
            ...
            case 3: {
                for(;;){
                    try{
                        ...
                        s[n].Postavi(s[n].DajAdresu(), s[n].DajSprat(),brSoba, s[n].DajNamjesten(),s[n].DajKvadraturu()); break;
                    }catch(std::domain_error err){
                        std::cout << "Izuzetak: " << err.what() << ". Molimo unesite opet." << std::endl;
                    }
                }
            ...
            case 4:{
                for(;;){
                    try{
                        int namj; std::cin >> namj;
                        s[n].Postavi(s[n].DajAdresu(),s[n].DajSprat(),s[n].DajBrojSoba(),namj,s[n].DajKvadraturu()); break;
                    }catch(std::domain_error err){
                        std::cout << "Izuzetak: " << err.what() << ". Molimo unesite opet." << std::endl;
                    }
                }
            ...
            case 5:{
                for(;;){
                    try{
                        double kv; std::cin >> kv;
                        s[n].Postavi(s[n].DajAdresu(),s[n].DajSprat(), s[n].DajBrojSoba(), s[n].DajNamjesten(), kv); break;
                    }catch(std::domain_error err){
                        std::cout << "Izuzetak: " << err.what() << ". Molimo unesite opet." << std::endl;
                    }
                }
            ...
            case 6:{
                for(;;){
                    try{
                        std::cout << "Unesite adresu: "; std::cin.ignore(100,'\n');
                        std::string adr; std::getline(std::cin,adr);
                        std::cout << "Unesite sprat: ";
                        int sprat; std::cin >> sprat;
                        std::cout << "Unesite broj soba: ";
                        int soba; std::cin >> soba;
                        ...
                        std::cout << "Unesite kvadraturu: ";
                        double kvad; std::cin >> kvad;
                        s[n].Postavi(adr,sprat,soba,namj,kvad);
