...
void Prodaja::Ispisi() const {
    std::cout << std::left << std::setw(32) << "Ime agenta: " << std::right << std::setw(5) << ime_agenta_prodaje
        << std::endl << std::left << std::setw(32) << "Ime kupca: ";
    kupac_stana.Ispisi();
    std::cout << std::endl << std::left << std::setw(32) << "Zakazani datum prodaje: ";
    datum_prodaje.Ispisi();
    std::cout << std::endl << std::left << std::setw(32) << "Cijena stana: " << std::right << std::setw(5) << cijena_stana << std::endl;
...
Prodaje::Prodaje(int max_broj_prodaja) : max_broj_prodaja(max_broj_prodaja),
    broj_prodaja(0), prodaje(new Prodaja*[max_broj_prodaja]) {}
    
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja) : max_broj_prodaja(spisak_prodaja.size()),
    broj_prodaja(spisak_prodaja.size()), prodaje(new Prodaja*[max_broj_prodaja]) {
    int i(0);
...
Prodaje::Prodaje(const Prodaje &prodaje) : max_broj_prodaja(prodaje.max_broj_prodaja), broj_prodaja(prodaje.broj_prodaja),
    prodaje(new Prodaja *[prodaje.max_broj_prodaja]) {
    try {
        for(int i(0); i<prodaje.broj_prodaja; i++)
            Prodaje::prodaje[i]=new Prodaja(*prodaje.prodaje[i]);
    }
    catch(...) {
        for(int i(0); i<prodaje.broj_prodaja; i++) delete Prodaje::prodaje[i];
        delete[] Prodaje::prodaje; throw;
    }
}

Prodaje &Prodaje::operator =(const Prodaje &prodaje) {
    if(max_broj_prodaja!=prodaje.max_broj_prodaja) throw std::logic_error("Prodaje nesaglasnih kapaciteta");
    if(prodaje.broj_prodaja>broj_prodaja) {
        try {
            ...
            for(int i(broj_prodaja); i<prodaje.broj_prodaja; i++) {
                delete Prodaje::prodaje[i]; Prodaje::prodaje[i]=nullptr;
            }
        }
    } else {
    ...
    for(int i(0); i<broj_prodaja; i++) *Prodaje::prodaje[i]=*prodaje.prodaje[i];
    return *this;
}
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, const Datum &datum_prodaje,
        const Kupac &kupac_stana, const Stan &kupljeni_stan) {
            ...
            prodaje[broj_prodaja++]=new Prodaja(p1);
}
...
Prodaja &Prodaje::DajProdaju(int redni_broj) const {
    if(redni_broj>broj_prodaja) throw std::domain_error("Redni broj nije validan");
...
int Prodaje::DajBrojProdajaNaDatum(const Datum &datum) const {
    return std::count_if(prodaje, prodaje+broj_prodaja,
        [datum] (const Prodaja *p) {if(p->DajDatumProdaje().DajDan()==datum.DajDan()
...
int Prodaje::DajBrojProdajaOdAgenta(const std::string &ime_agenta) const {
    return std::count_if(prodaje, prodaje+broj_prodaja, [ime_agenta] (const Prodaja *p) {if(p->DajImeAgenta()==ime_agenta) return true; return false;});
}
...
Prodaja &Prodaje::DajNajranijuProdaju() {
    ...
    return **std::min_element(prodaje, prodaje+broj_prodaja,
        [] (const Prodaja *p1, const Prodaja *p2) {
...
Prodaja Prodaje::DajNajskupljuProdaju() const {
    if(broj_prodaja==0) throw std::domain_error("Nema registriranih prodaja");
    Prodaja p = **std::max_element(prodaje, prodaje+broj_prodaja,
        [] (const Prodaja *p1, const Prodaja *p2) {
            if(p1->DajCijenuStana() < p2->DajCijenuStana()) return false;
            else if(p1->DajCijenuStana() > p2->DajCijenuStana()) return true;
            return false;
...
void Prodaje::ObrisiNajranijuProdaju() {
    ...
    for(int i(0); i<broj_prodaja; i++) if(!prodaje[i]) {throw std::range_error("Prazna kolekcija");}
    for(int i(0); i<broj_prodaja; i++)
...
void Prodaje::IspisiSveProdaje() const {
    ...
    for(int i(0); i<nove.size(); i++) {
        ...
        nove[i].Ispisi(); std::cout << std::endl << std::endl;
    }
}
