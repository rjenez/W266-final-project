...
class Pregled{
  ...
  void PromjeniPacijenta(const std::string &ime_pacijenta) {this->ime=ime_pacijenta;}
...
void Pregled::PomjeriDanUnaprijed(){
  ...
  a[0]++;
  if(a[0]>broj_dana[a[1]-1]){a[0]=1;a[1]++;}
  if(a[1]>12){a[1]=1;a[2]++;}
  ...
  elementi(new Pregled*[spisak_pregleda.size()]){
    ...
    for(int i=0;i<spisak_pregleda.size();i++){
      ...
      it++;
        ...
        for(int i=0;i<spisak_pregleda.size();i++)delete elementi[i];delete[] elementi;
      }
    }
  }
  ~Pregledi(){
  ...
  Pregledi(const Pregledi &pregledi):max(pregledi.max),broj(pregledi.broj),elementi(new Pregled*[pregledi.max]{}){
    for(int i=0;i<max;i++){
      try{
        ...
        for(int i=0;i<max;i++) delete elementi[i]; delete[] elementi;throw;
      }
    }
  }
...
void Pregledi::IsprazniKolekciju(){
  for(int i=0;i<broj;i++){delete elementi[i];elementi[i]=nullptr;}
  broj=0;
}
...
void Pregledi::ObrisiNajranijiPregled(){
  ...
  std::for_each(elementi,elementi+broj,[this,&a](Pregled*p){if(!JednakiPregledi(this->DajNajranijiPregled(),*p))a.push_back(*p);});
  for(int i=0;i<broj;i++) delete elementi[i];
...
void Pregledi::ObrisiPregledePacijenta(const std::string &ime_pacijenta){
  ...
  std::vector<Pregled> a;
  std::for_each(elementi,elementi+broj,[&a,ime_pacijenta](Pregled*p){if(ime_pacijenta!=p->DajImePacijenta())a.push_back(*p);});
  for(int i=0;i<broj;i++) delete elementi[i];
  for(int i=0;i<a.size();i++) elementi[i]=new Pregled(a[i]);
  broj=a.size();
      }
...
void Pregledi::IspisiSvePreglede() const{
  std::vector<Pregled> a;
   std::for_each(elementi,elementi+broj,[&a](Pregled*pregled){a.push_back(*pregled);});
    std::sort(a.begin(),a.end(),[](Pregled p1,Pregled p2)->bool{
    if(std::get<2>(p1.DajDatumPregleda().Ocitaj())<std::get<2>(p2.DajDatumPregleda().Ocitaj()))return true;
    else if(std::get<2>(p1.DajDatumPregleda().Ocitaj())>std::get<2>(p2.DajDatumPregleda().Ocitaj()))return false;
    else if(std::get<2>(p1.DajDatumPregleda().Ocitaj())==std::get<2>(p2.DajDatumPregleda().Ocitaj())) {
