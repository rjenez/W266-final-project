...
class Stan{
    ...
    void Ispisi() const{
        ...
        if(Namjesten)std::cout<<"je namjesten."<<std::endl;if(!Namjesten)std::cout<<"nije namjesten."<<std::endl;
    }
};
class Prodaja{
...
class Prodaje{
    int Max_broj;
    Prodaja **pok_prodaje; 
    mutable int broj_evidentiranih;
    public:
    explicit Prodaje( int max_broj_prodaja):Max_broj(max_broj_prodaja),broj_evidentiranih(0){
        try{
        pok_prodaje=new Prodaja*[max_broj_prodaja]{nullptr};
        }catch(...){throw std::domain_error ("Problem sa memorijom");}
    }
    Prodaje(std::initializer_list<Prodaja> spisak_prodaja) : Max_broj(spisak_prodaja.size()){
        try{
            pok_prodaje=new Prodaja*[Max_broj]{nullptr};
            broj_evidentiranih=Max_broj;
            int brojac=0;
            for(auto i:spisak_prodaja){
                pok_prodaje[brojac]=new Prodaja(i.DajImeAgenta(),i.DajCijenuStana(),i.DajDatumProdaje(),
    ...
    Prodaja &DajProdaju(int i){return *pok_prodaje[i];}
    ...
    void IspisiSveProdaje(int i=0) const;
};
Prodaje::Prodaje(const Prodaje &p){
    ...
    pok_prodaje=new Prodaja*[Max_broj]{nullptr};
    for(int i=0;i<Max_broj;i++){
            pok_prodaje[i]=new Prodaja(p.pok_prodaje[i]->DajImeAgenta(),
            p.pok_prodaje[i]->DajCijenuStana(),p.pok_prodaje[i]->DajDatumProdaje(),
            p.pok_prodaje[i]->DajKupca(),p.pok_prodaje[i]->DajKupljeniStan());
        }
    }
Prodaje &Prodaje::operator =(const Prodaje &p){
    broj_evidentiranih=p.broj_evidentiranih;
    for(int i=0;i<p.broj_evidentiranih;i++){delete pok_prodaje[i];pok_prodaje[i]=nullptr;}
        for(int i=0;i<Max_broj;i++){
            pok_prodaje[i]=new Prodaja(p.pok_prodaje[i]->DajImeAgenta(),
            p.pok_prodaje[i]->DajCijenuStana(),p.pok_prodaje[i]->DajDatumProdaje(),
            p.pok_prodaje[i]->DajKupca(),p.pok_prodaje[i]->DajKupljeniStan());
...
Prodaje::Prodaje(Prodaje &&p){
    ...
    for(int i=0;i<Max_broj;i++){
            ...
            p.pok_prodaje[i]->DajCijenuStana(),p.pok_prodaje[i]->DajDatumProdaje(),
            p.pok_prodaje[i]->DajKupca(),p.pok_prodaje[i]->DajKupljeniStan());
            delete p.pok_prodaje[i];
        }
    delete p.pok_prodaje;
}
Prodaje &Prodaje::operator =(Prodaje &&p){
    for(int i=0;i<Max_broj;i++){delete pok_prodaje[i];pok_prodaje[i]=nullptr;}
    broj_evidentiranih=p.broj_evidentiranih;
    for(int i=0;i<Max_broj;i++){
            pok_prodaje[i]=new Prodaja(p.pok_prodaje[i]->DajImeAgenta(),
            p.pok_prodaje[i]->DajCijenuStana(),p.pok_prodaje[i]->DajDatumProdaje(),
...
void Prodaje::IzmjeniProdaju(std::string &ime_agenta,double cijena,Datum &datum_prodaje,
    Kupac  &kupac,Stan &stan,int index){
        delete pok_prodaje[index];
        pok_prodaje[index]=new Prodaja(ime_agenta,cijena,datum_prodaje,kupac,stan);
    }
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje,
    double cijena_stana, const Datum &datum_prodaje, const Kupac &kupac_stana,
    const Stan &kupljeni_stan){//provjeriti ime agenta i datume ime kupca stana
        if(broj_evidentiranih>=Max_broj)throw std::range_error ("Dostignut maksimalni broj prodaja");
        try{
        pok_prodaje[broj_evidentiranih]=new Prodaja(ime_agenta_prodaje,cijena_stana,datum_prodaje,
        kupac_stana,kupljeni_stan);
        }
        catch(std::domain_error x){throw x;}
        broj_evidentiranih+=1;
    }
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana,
    ...
    int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata){
        if(broj_evidentiranih>=Max_broj)throw std::range_error("Dostignut maksimalni broj prodaja");
        try{
        pok_prodaje[broj_evidentiranih]=new Prodaja(ime_agenta_prodaje,
...
void Prodaje::RegistrirajProdaju(Prodaja *prodaja){
    if(broj_evidentiranih>=Max_broj)throw std::range_error("Dostignut maksimalni broj prodaja");
    try{
        pok_prodaje[broj_evidentiranih]=new Prodaja(prodaja->DajImeAgenta(),
...
int Prodaje::DajBrojProdajaNaDatum(const Datum &datum) const{
    int brojac(0);
    std::count_if(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[datum,&brojac](Prodaja *a){
...
int Prodaje::DajBrojProdajaOdAgenta(const std::string &ime_agenta) const{
    int brojac(0);
    std::count_if(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[ime_agenta,&brojac](Prodaja *a){
        if(ime_agenta==a->DajImeAgenta()){brojac++;return true;}
        return false;
    });
    return brojac;
}
Prodaja Prodaje::DajNajranijuProdaju() const{
    if(broj_evidentiranih==0)throw std::domain_error("Nema registriranih prodaja");
    Prodaja **pok=std::min_element(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[](Prodaja *a,
    Prodaja *b){
...
Prodaja &Prodaje::DajNajskupljuProdaju(){
    if(broj_evidentiranih==0)throw std::domain_error("Nema registriranih prodaja");
    Prodaja **pok=std::max_element(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[]
    (Prodaja *a,Prodaja *b){
...
void Prodaje::IsprazniKolekciju(){
    ...
    for(int i=0;i<broj_evidentiranih;i++){
        delete pok_prodaje[i];
        pok_prodaje[i]=nullptr;
    }
}
void Prodaje::ObrisiNajranijuProdaju(){
    if(broj_evidentiranih==0)throw std::range_error("Prazna kolekcija");
    auto p=DajNajranijuProdaju();
    std::for_each(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[p](Prodaja *a){
        if(a==&p){delete a;a=nullptr;}
    });
}
void Prodaje::ObrisiProdajeAgenta(const std::string &ime_agenta){
    if(broj_evidentiranih==0)throw std::range_error("Prazna kolekcija");
    std::for_each(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[ime_agenta]
    (Prodaja *a){
        if(ime_agenta==a->DajImeAgenta()){delete a;a=nullptr;}
    });
}
void Prodaje::ObrisiProdajeNaDatum(const Datum &datum){
    if(broj_evidentiranih==0)throw std::range_error("Prazna kolekcija");
    std::for_each(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[datum]
    (Prodaja *a){
        if(a->DajDatumProdaje().DajDan()==datum.DajDan() && 
        a->DajDatumProdaje().DajMjesec()==datum.DajMjesec() &&
        a->DajDatumProdaje().DajGodinu()==datum.DajGodinu()){delete a;a=nullptr;}
    });
}
void Prodaje::IspisiProdajeNaDatum(const Datum &datum) const{
    std::sort(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[](Prodaja *a,
    Prodaja *b){
        return ProdatPrije(*a,*b);
    });
    std::for_each(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[datum]
...
void Prodaje::IspisiSveProdaje(int i) const{
    int brojac=1;
    if(i==1){
        std::sort(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[]
        (Prodaja *a,Prodaja *b){
            return ProdatPrije(*a,*b);
        });
    }
    std::for_each(&pok_prodaje[0],&pok_prodaje[broj_evidentiranih],[&brojac,i]
    (Prodaja *a){
