...
class Prodaje {
    ...
    void Realociraj();
public:
    explicit Prodaje(int max_broj_prodaja) : max_br_prodaja(max_broj_prodaja) {
        prodaje = new Prodaja*[max_br_prodaja]{};
    }
    Prodaje(std::initializer_list<Prodaja> spisak_prodaja);
    ~Prodaje() {
        for(int i = 0; i < max_br_prodaja; i++)
            delete prodaje[i];
        delete[] prodaje;
        max_br_prodaja = 0; prodaje = nullptr;
    }
    Prodaje(const Prodaje &prodaje2);
    Prodaje(Prodaje &&prodaje2) : max_br_prodaja(prodaje2.max_br_prodaja) {
        prodaje = prodaje2.prodaje;
        prodaje2.prodaje = nullptr;
    }
    Prodaje &operator = (const Prodaje &prodaje2) {
        for(int i = 0; i < max_br_prodaja; i++)
            delete prodaje[i];
        delete[] prodaje;
        max_br_prodaja = prodaje2.max_br_prodaja;
        prodaje = new Prodaja* [max_br_prodaja];
        for(int i = 0; i < max_br_prodaja; i++)
            prodaje[i] = new Prodaja(*prodaje2.prodaje[i]);
        return *this;
    }
...
void Prodaje::Realociraj() {
    Prodaja** novo_prodaje(nullptr);
    try{
        novo_prodaje = new Prodaja*[max_br_prodaja]{};
        int broj_uspjesnih(0);
        for(int i = 0; i < max_br_prodaja; i++)
            if(prodaje[i]) {
                novo_prodaje[broj_uspjesnih] = new Prodaja (*prodaje[i]);
                broj_uspjesnih++;
            }
        for(int i = 0; i < max_br_prodaja; i++)
            delete prodaje[i];
...
Prodaje::Prodaje(const Prodaje &prodaje2) {
    prodaje = new Prodaja*[max_br_prodaja]{};
    try{
        for(int i = 0; i < max_br_prodaja; i++)
            prodaje[i] = new Prodaja(*prodaje2.prodaje[i]);
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, const Datum &datum_prodaje, const Kupac &kupac_stana, const Stan &kupljeni_stan) {
    if(max_br_prodaja == DajBrojProdaja()) throw std::range_error("Dostignut maksimalan broj prodaja");
    prodaje[Prodaje::DajBrojProdaja()] = new Prodaja(Prodaja(ime_agenta_prodaje, cijena_stana, datum_prodaje, kupac_stana, kupljeni_stan));
}
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, int dan_prodaje, int mjesec_prodaje, int godina_prodaje, std::string &ime_kupca,
        const Datum &datum_rodjenja_kupca, const std::string &adresa_stana, int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata) {
            if(max_br_prodaja == Prodaje::DajBrojProdaja()) throw std::range_error("Dostignut maksimalan broj prodaja");
            ...
            prodaje[Prodaje::DajBrojProdaja()] = new Prodaja(Prodaja(ime_agenta_prodaje, cijena_stana, tmp_datum, tmp_kupac, tmp_stan));
}
void Prodaje::RegistrirajProdaju(Prodaja *prodaja) {
    if(max_br_prodaja == DajBrojProdaja()) throw std::range_error("Dostignut maksimalan broj prodaja");
    prodaje[DajBrojProdaja()] = new Prodaja(*prodaja);
}
...
int Prodaje::DajBrojProdajaNaDatum(const Datum &datum) const {
    ...
    return std::count_if(prodaje, prodaje+DajBrojProdaja(), [datum](Prodaja* pr){
...
int Prodaje::DajBrojProdajaOdAgenta(const std::string &ime_agenta) const {
    if(!DajBrojProdaja()) throw std::domain_error("Nema registriranih prodaja");
    return std::count_if(prodaje, prodaje+DajBrojProdaja(), [ime_agenta] (Prodaja* pr) { return pr->DajImeAgenta() == ime_agenta;});
}
Prodaja& Prodaje::DajNajranijuProdaju() {
    if(!DajBrojProdaja()) throw std::domain_error("Nema registriranih prodaja");
    Prodaja**  prod (std::min_element(prodaje, prodaje+DajBrojProdaja(), [](Prodaja* p1, Prodaja* p2){return ProdatPrije(*p1, *p2);}));
    return **prod;
}
...
Prodaja& Prodaje::DajNajskupljuProdaju() {
    if(!DajBrojProdaja()) throw std::domain_error("Nema registriranih prodaja");
    Prodaja** prod (std::max_element(prodaje, prodaje+DajBrojProdaja(), [](Prodaja* p1, Prodaja* p2){return p2->DajCijenuStana() > p1->DajCijenuStana();}));
    return **prod;
}
...
Prodaja& Prodaje::DajProdaju(int indeks) {
    if(indeks < 1 || indeks > max_br_prodaja || !prodaje[indeks-1]) throw std::logic_error("Redni broj nije ispravan;");
...
void Prodaje::ObrisiNajranijuProdaju() {
    if(!DajBrojProdaja()) throw std::range_error("Prazna kolekcija");
    auto prod(std::min_element(prodaje, prodaje+DajBrojProdaja(), [](Prodaja* p1, Prodaja* p2){return ProdatPrije(*p1, *p2);}));
    delete *prod;
    *prod = nullptr;
    Realociraj();
}
...
void Prodaje::IspisiProdajeNaDatum(const Datum &datum) const {
    ...
    std::sort(prodaje2, prodaje2+DajBrojProdaja(), [](Prodaja* p1, Prodaja* p2) {return p1->DajImeKupca() < p2->DajImeKupca();});
    for(int i = 0; i < max_br_prodaja; i++)
        if(prodaje2[i] && PorediDatum(prodaje2[i]->DajDatumProdaje(), datum)) { prodaje2[i]->Ispisi(); std::cout << std::endl << std::endl;}
}
void Prodaje::IspisiSveProdaje() const {
    auto prodaje2 = prodaje;
    std::sort(prodaje2, prodaje2+DajBrojProdaja(), [](Prodaja* p1, Prodaja* p2) {
