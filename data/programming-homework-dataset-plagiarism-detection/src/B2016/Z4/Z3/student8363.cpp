...
class Prodaje
{
    ...
    Prodaja **p=nullptr;
    int max_p = 0 ;
    void throwerr(int)const;
    void DodajProdaju(Prodaja);
    void unistiJedanElement(Prodaja *q);
    void unistiElemente();
    void dubokaKopija(const Prodaje&);
    void plitkaKopija(Prodaje&);
    void dodajProdaju(const string&,double,const Datum&,const Kupac&,const Stan&);
public :
    explicit Prodaje(int);
    Prodaje(std::initializer_list<Prodaja> sp_p);
    ~Prodaje();
    Prodaje(const Prodaje&);
    Prodaje(Prodaje&&);
    ...
    void RegistrirajProdaju(const string&,double,const Datum&,const Kupac&,const Stan&);
    void RegistrirajProdaju(const string&,double,int,int,int,string&,const Datum&,const string&,int,int,bool,double);
    void RegistrirajProdaju(Prodaja*);
    int DajBrojProdaja()const;
    int DajBrojProdajaNaDatum(const Datum&)const;
    int DajBrojProdajaOdAgenta(const string&)const;
    ...
    void ObrisiProdajeAgenta(const string&);
    void ObrisiProdajeNaDatum(const Datum&);
    void IspisiProdajeNaDatum(const Datum&)const;
    void IspisiSveProdaje()const;
};
    void Prodaje::throwerr(int i=0) const{
        switch(i){
            case 0:
    ...
    void Prodaje::DodajProdaju(Prodaja P) {
        //test curenja mem
    ...
    void Prodaje::unistiJedanElement(Prodaja *q){
    ...
    void Prodaje::unistiElemente(){
        for (int i = 0; i < poz_z; i++) {
            delete p[i];
            p[i]=nullptr;
        }
    }
    void Prodaje::dubokaKopija(const Prodaje &px){
        unistiElemente();
        delete[] p;
    ...
    void Prodaje::plitkaKopija(Prodaje &px){
        unistiElemente();
        delete[] p;
        p = px.p;
        max_p = px.max_p;
    ...
    void Prodaje::dodajProdaju(const string &a_p, double c, const Datum &d_p, const Kupac &k, const Stan &s){
        if(poz_z>=max_p)throwerr();
        try{
    ...
    Prodaje::Prodaje(int m_p):max_p(m_p) {
        //predvidjet za m_p<=0
        p = new Prodaja*[max_p];
        poz_z=0;
    }
    Prodaje::Prodaje(std::initializer_list<Prodaja> sp_p) {
        if(p) {
    ...
    Prodaje::Prodaje(const Prodaje &px) {
        dubokaKopija(px);
    }
    Prodaje::Prodaje(Prodaje &&px){
        plitkaKopija(px);
    }
    void Prodaje::RegistrirajProdaju(const string &a_p, double c, const Datum &d_p, const Kupac &k, const Stan &s){
        dodajProdaju(a_p,c,d_p,k,s);
    }
    void Prodaje::RegistrirajProdaju(const string &a_p, double c, int d, int m, int g, string &i_k, const Datum &d_r, const string &adr,int s, int b_s,bool n,double k){
    ...
    int Prodaje::DajBrojProdajaOdAgenta(const string &i_a)const{
    ...
    void Prodaje::ObrisiProdajeAgenta(const string &i_a){
        for (int i = 0; i < poz_z; i++) {
    ...
    void Prodaje::IspisiProdajeNaDatum(const Datum &d)const{
...
Datum uDatum(string s,bool catcherr = false){
    cout<<s;
    int d,m,g;
    cin>>d;
    cin.ignore(1);
...
int UredjivanjeStana_uredjivanje_stanova(int &n,vector<Stan> &vs){
    ...
    while (true) {
        switch(n){
            case 0: n=1;return UredjivanjeStana(n);
            case 6:
    ...
    if(n==1){
        while (true) {
            try{
                ...
                int c= uCijenuS();
                Datum d = uDatum("Unesite datum prodaje (formata d/m/g): ", true);
        ...
        while (true) {
            try{
                ...
                cin>>n;
                string ime = uImeAgenta();
                Datum dp = uDatum("Unesite datum prodaje (formata d/m/g): ",true);
                int c = uCijenuS();
                ...
                cin>>n;
                Stan st = vs[n-1];
                ps.RegistrirajProdaju(ime,c,dp,k,st);
                return 1;
            }catch(std::domain_error e){
                cout<<"Izuzetak "<<e.what()<<". Molimo unesite ponovo.\n"<<endl;
            }
