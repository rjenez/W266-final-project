...
{
private:
int cmax_broj_prodaja;
int cbroj_prodaja;
Prodaja **cpok=nullptr;
public:
...
int DajBrojProdaja() const{return cbroj_prodaja;}
...
Prodaje::Prodaje(int max_broj_prodaja) : cmax_broj_prodaja(max_broj_prodaja),cbroj_prodaja(0),cpok(new Prodaja*[max_broj_prodaja]) {}
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja) : cmax_broj_prodaja(spisak_prodaja.size()), cpok(new Prodaja*[spisak_prodaja.size()])
{
    int i(0),pom(0);
    try
    {
...
Prodaje::~Prodaje()
{
    if(cpok!=nullptr)
        {
            for(int j=0;j<cbroj_prodaja;j++)
                delete cpok[j];
                delete[] cpok;
                cpok=nullptr;
        }
}
Prodaje::Prodaje(const Prodaje &prodaje) :cmax_broj_prodaja(prodaje.cmax_broj_prodaja),cbroj_prodaja(prodaje.cbroj_prodaja),cpok(new Prodaja*[prodaje.cmax_broj_prodaja])
{
    int pom(0);
    try
    {
        for(int i=0;i<prodaje.cbroj_prodaja;i++)
        {
            cpok[i]=new Prodaja(*prodaje.cpok[i]);
            pom++;
        }
    }
    catch(...)
...
{
    cbroj_prodaja=prodaje.cbroj_prodaja;
    cmax_broj_prodaja=prodaje.cmax_broj_prodaja;
    cpok=prodaje.cpok;
    prodaje.cpok=nullptr;
    prodaje.cbroj_prodaja=0;
}
Prodaje &Prodaje::operator =(const Prodaje &prodaje) //
{
    ...
    Prodaja **npok=new Prodaja*[prodaje.cmax_broj_prodaja]{};
    try
    {
    for(int i=0;i<prodaje.cbroj_prodaja;i++)
    npok[i]=new Prodaja(*prodaje.cpok[i]);
    }
    catch(...)
    {
    for(int i=0;i<prodaje.cbroj_prodaja;i++)
...
Prodaje &Prodaje::operator =(Prodaje &&prodaje) //
{
    if(&prodaje!=this)
    {
        std::swap(cbroj_prodaja,prodaje.cbroj_prodaja);
        std::swap(cmax_broj_prodaja,prodaje.cmax_broj_prodaja);
        std::swap(cpok,prodaje.cpok);
    }
    return *this;
}
...
{
    if(cbroj_prodaja==cmax_broj_prodaja)
    throw std::range_error("Dostignut maksimalni broj prodaja");
    else
    {
    cpok[cbroj_prodaja]=prodaja;
    cbroj_prodaja++;
    }
}
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, const Datum &datum_prodaje, const Kupac &kupac_stana, const Stan &kupljeni_stan)
{
    ...
    try
    {
        cpok[cbroj_prodaja]=new Prodaja(ime_agenta_prodaje,cijena_stana,datum_prodaje,kupac_stana,kupljeni_stan);
        cbroj_prodaja++;
    }
    catch(...)
    {
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje,double cijena_stana, int dan_prodaje, int mjesec_prodaje, int godina_prodaje, std::string &ime_kupca, const Datum &datum_rodjenja_kupca, const std::string &adresa_stana, int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata)
{
    ...
    try
    {
        cpok[cbroj_prodaja]=new Prodaja(ime_agenta_prodaje,cijena_stana,dan_prodaje,mjesec_prodaje,godina_prodaje,ime_kupca,datum_rodjenja_kupca,adresa_stana,sprat_stana,broj_soba,namjesten_stan,broj_kvadrata);
        cbroj_prodaja++;
    }
    catch(...)
    {
...
{
    int pom;
    pom=std::count_if(cpok,cpok+cbroj_prodaja,[=](Prodaja *prodaja) {
...
Prodaja &Prodaje::DajNajranijuProdaju()
{
    ...
    return **std::min_element(cpok,cpok+cbroj_prodaja,[] (Prodaja *prodaja1,Prodaja *prodaja2)
    {
...
Prodaja &Prodaje::DajNajskupljuProdaju()
{
    if(cbroj_prodaja==0)
    throw std::domain_error("Nema registriranih prodaja");
    return **std::max_element(cpok,cpok+cbroj_prodaja,[](Prodaja *prodaja1,Prodaja *prodaja2)
    {
...
void Prodaje::ObrisiNajranijuProdaju()
{
   ...
   for(int i=0;i<cbroj_prodaja;i++)
   {
       ...
       {
           cpok[i]=cpok[cbroj_prodaja-1];
           delete cpok[cbroj_prodaja-1];
           cbroj_prodaja--;
       }
   }
...
void Prodaje::IspisiSveProdaje() const
{
    ...
    std::sort(pom_prodaje.cpok,pom_prodaje.cpok+cbroj_prodaja,[](Prodaja *prodaja1,Prodaja *prodaja2)
    {
         Datum datum1=prodaja1->Prodaja::DajDatumProdaje(),datum2=prodaja2->Prodaja::DajDatumProdaje();
        if(datum1.Datum::DajGodinu()<datum2.Datum::DajGodinu()) return true;
        if(datum1.Datum::DajGodinu()>datum2.Datum::DajGodinu()) return false;
