...
class Prodaje {
    Prodaja **niz;
    int broj_prodaja, maks;
public:
    explicit Prodaje(int max_broj_prodaja) { niz=new Prodaja*[max_broj_prodaja]; maks=max_broj_prodaja; broj_prodaja=0; } //
    ...
    Prodaja& DajProdaju(int indeks) {
        if(indeks<0 || indeks>=broj_prodaja) throw std::range_error("...");
        return *niz[indeks];
    }
};
...
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja) {
    ...
    for(auto x : spisak_prodaja) 
        niz[i++]=&x;
    maks=broj_prodaja=spisak_prodaja.size();
}
Prodaje::Prodaje(const Prodaje &p) {
    broj_prodaja=p.broj_prodaja;
    maks=p.maks;
    niz=new Prodaja*[maks];
    for(int i=0; i<broj_prodaja; i++) {
        try { niz[i]=new Prodaja(*p.niz[i]); }
        catch(std::bad_alloc) {
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, const Datum &datum_prodaje, const Kupac &kupac_stana, const Stan &kupljeni_stan) {
    if(maks==broj_prodaja) throw std::range_error("Dostignut maksimalni broj prodaja");
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, int dan_prodaje, int mjesec_prodaje, int godina_prodaje, std::string &ime_kupca, const Datum &datum_rodjenja_kupca, const std::string &adresa_stana, int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata) {
    if(maks==broj_prodaja) throw std::range_error("Dostignut maksimalni broj prodaja");
    niz[broj_prodaja]=new Prodaja(ime_agenta_prodaje, cijena_stana, Datum(dan_prodaje, mjesec_prodaje, godina_prodaje), Kupac(ime_kupca, datum_rodjenja_kupca), Stan(adresa_stana, sprat_stana, broj_soba, namjesten_stan, broj_kvadrata));
    broj_prodaja++;
}
...
int Prodaje::DajBrojProdajaNaDatum(const Datum &datum) const {
    return std::count_if(niz, niz+broj_prodaja, [datum](const Prodaja *p) {
        auto x=p->DajDatumProdaje();
        if(x.DajDan()==datum.DajDan() && x.DajMjesec()==datum.DajMjesec() && x.DajGodinu()==datum.DajGodinu()) return true;
        return false;
    });
}
int Prodaje::DajBrojProdajaOdAgenta(const std::string &ime_agenta) const {
    return std::count_if(niz, niz+broj_prodaja, [ime_agenta](const Prodaja *p) {
        return p->DajImeAgenta()==ime_agenta;
    });
}

Prodaja &Prodaje::DajNajranijuProdaju() {
    auto x=std::min_element(niz, niz+broj_prodaja, [](const Prodaja *p1, const Prodaja *p2) {
       return ProdatPrije(*p1, *p2); 
    });
    if(x==niz+broj_prodaja) throw std::domain_error("Nema registriranih prodaja");
    return **x;
}
...
Prodaja &Prodaje::DajNajskupljuProdaju() {
    auto x=std::max_element(niz, niz+broj_prodaja, [](const Prodaja *p1, const Prodaja *p2) {
...
void Prodaje::IsprazniKolekciju() {
    for(int i=0; i<broj_prodaja; i++) delete niz[i];
    delete[] niz; maks=0; broj_prodaja=0;
}
...
void Prodaje::ObrisiNajranijuProdaju() {
    ...
    if(x==niz+broj_prodaja) throw std::range_error("Prazna kolekcija");
    delete x;
    for(int i=x-niz; i<broj_prodaja-1; i++) {
        niz[i]=niz[i+1];
    }
    broj_prodaja--;
}
...
void Prodaje::IspisiProdajeNaDatum(const Datum &datum) const {
    Prodaje p(*this);
    Prodaja **pomocna=p.niz;
    std::sort(pomocna, pomocna+broj_prodaja, [](const Prodaja *p1, const Prodaja *p2) {
    ...
    for(int i=0; i<broj_prodaja; i++) {
        auto x=pomocna[i]->DajDatumProdaje();
        if(x.DajDan()==datum.DajDan() && x.DajMjesec()==datum.DajMjesec() && x.DajGodinu()==datum.DajGodinu()) pomocna[i]->Ispisi();
        std::cout << "\n";
    }
}
...
int main () {
    Prodaje sve(20); std::vector<Stan>stanovi; std::vector<Kupac>kupci;
	std::cout << "Dobrodosli!\n\n";
	for(;;) {
