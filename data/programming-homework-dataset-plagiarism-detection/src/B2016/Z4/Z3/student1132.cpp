...
{
    if(ind > brel || ind < 1) throw "losina";
 ...
 Prodaje::Prodaje(int max_broj_prodaja)
{
        ...
        for(int i=0;i<max_broj_prodaja; i++) prodaje[i] = nullptr;
        max_brel = max_broj_prodaja;
        brel = 0;
}
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja)
{
    ...
    max_brel = spisak_prodaja.size();
    brel = spisak_prodaja.size();
}
Prodaje::~Prodaje()
{
    for(int i=0;i<brel;i++) delete prodaje[i];
    delete[] prodaje;
}
Prodaje::Prodaje(const Prodaje& p)
{
    ...
    {
        prodaje = nullptr;
        prodaje = new Prodaja*[p.max_brel];
        for(int i=0; i<p.max_brel; i++) prodaje[i] = nullptr;
        for(int i=0; i<p.brel; i++)
        {
    ...
    {
        for(int i=0; i < p.max_brel; i++) delete prodaje[i];
        delete[] prodaje;
        throw;
    }
}
...
{
    if(max_brel < p.max_brel)
    {
        Prodaja** novo;
        try
        {
        novo = nullptr;
        novo = new Prodaja*[p.max_brel];
        for(int i=0; i < p.max_brel; i++) novo[i] = nullptr;
        for(int i=0; i<p.brel; i++) novo[i] = new Prodaja(*p.prodaje[i]);
        for(int i=0; i<brel; i++) delete prodaje[i];
        delete[] prodaje;
        prodaje = novo;
        brel = p.brel;
        ...
        {
            for(int i=0;i<p.brel;i++) delete novo[i];
            delete[] novo;
            throw;
        }
    }
    else
    {
        try
        {
        for(int i=0; i<brel; i++) {delete prodaje[i]; prodaje[i]=nullptr; }
        for(int i=0; i<p.brel; i++) prodaje[i] = new Prodaja(*p.prodaje[i]);
        max_brel = p.max_brel;
        brel = p.brel;
        }
        catch(...)
        {
            for(int i=0;i<p.brel;i++) delete prodaje[i];
            throw;
        }
    }
    return *this;
...
{
   if(brel == max_brel) throw std::range_error("Dostignut maksimalni broj prodaja");
   prodaje[brel++] = new Prodaja(im, ci, da, ku, st);
}
...
{
    if(brel == max_brel) throw std::range_error("Dostignut maksimalni broj prodaja");
    prodaje[brel++] = new Prodaja(imea, ci, d, m, g, imek, datk, adr, sprat, bs, namj, brk);
}
...
int Prodaje::DajBrojProdaja() const { return brel; }
int Prodaje::DajBrojProdajaNaDatum(const Datum& d) const
{
    return std::count_if(prodaje, prodaje+brel, [d](Prodaja* p){ 
        auto e = p->DajDatumProdaje();
        return Jednaki(d,e);});
}
int Prodaje::DajBrojProdajaOdAgenta(const std::string &ag) const
{
    return std::count_if(prodaje, prodaje+brel, [ag](Prodaja* p){return p->DajImeAgenta() == ag;});
}
Prodaja& Prodaje::DajNajranijuProdaju()
{
    if(brel == 0) throw std::domain_error("Nema registriranih prodaja");
    return **std::min_element(prodaje, prodaje+brel, [](Prodaja* p1, Prodaja* p2){
...
Prodaja Prodaje::DajNajranijuProdaju() const {if(brel == 0) throw std::domain_error("Nema registriranih prodaja");
return Prodaje(*this).DajNajranijuProdaju(); }

Prodaja& Prodaje::DajNajskupljuProdaju()
{
    if(brel == 0) throw std::domain_error("Nema registriranih prodaja");
    return **std::max_element(prodaje, prodaje+brel, [](Prodaja* p1, Prodaja* p2){
...
{
    if(brel == 0) throw std::range_error("Prazna kolekcija");
    ...
    for(int i=0; i<brel; i++)
    {
        ...
        {
            delete prodaje[i];
            for(int j=i; j+1<brel; j++) prodaje[j] = prodaje[j+1];
            brel--;
            break;
        }
    }
...
{
    Prodaje aux(*this);
    std::sort(aux.prodaje, aux.prodaje+aux.brel, [](Prodaja* p1, Prodaja* p2){
