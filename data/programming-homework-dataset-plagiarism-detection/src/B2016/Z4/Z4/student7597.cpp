...
ime_agenta_prodaje(ime_agenta_prodaje),cijena_stana(cijena_stana),datum_prodaje(Datum(dan_prodaje,mjesec_prodaje,
...
Prodaje &operator =(const Prodaje &prodaje){
    p.resize(0);
    try{
        for(int i=0;i<prodaje.p.size();i++){
            p.emplace_back(std::make_shared<Prodaja>(*prodaje.p[i]));
        }
    }
    catch(...){
        p.resize(0);
...
void RegistrirajProdaju(const std::string &ime_agenta_prodaje,double cijena_stana, int dan_prodaje,int mjesec_prodaje, int godina_prodaje, std::string &ime_kupca,
 ...
 void RegistrirajProdaju(std::shared_ptr<Prodaja> prodaja){
     p.emplace_back(prodaja);
 } 
int DajBrojProdaja() const{
    return p.size();}
int DajBrojProdajaNaDatum(const Datum &datum) const{
     return std::count_if(p.begin(),p.end(),[datum](std::shared_ptr<Prodaja> pok){
        if(poredidvadatuma(pok->DajDatumProdaje(),datum)) return true;
        return false;
    });
}
int DajBrojProdajaOdAgenta(const std::string &ime_agenta) const{
    return std::count_if(p.begin(),p.end(),[ime_agenta](std::shared_ptr<Prodaja> pok){
        if(pok->DajImeAgenta()==ime_agenta) return true;
        return false;
    });
}
Prodaja &DajNajranijuProdaju(){
     auto pok=std::min_element(p.begin(),p.end(),[](std::shared_ptr<Prodaja>p1,std::shared_ptr<Prodaja> p2){
        return ProdatPrije(*p1,*p2);
    });
    if(pok==p.end()) throw std::domain_error("Nema registriranih prodaja");
    return **pok;
}
...
Prodaja &DajNajskupljuProdaju(){
     auto pok=std::max_element(p.begin(),p.end(),[](std::shared_ptr<Prodaja>p1,std::shared_ptr<Prodaja>p2){
...
void ObrisiNajranijuProdaju(){
    ...
    bool obrisan=false;
    for(int i=0;i<p.size();i++)if(p[i]->DajImeAgenta()==pok.DajImeAgenta()&&poredidvadatuma(p[i]->DajDatumProdaje(),pok.DajDatumProdaje())&&
    p[i]->DajCijenuStana()==pok.DajCijenuStana()) { p[i]=nullptr; p.erase(p.begin()+i); obrisan=true;}
...
void ObrisiProdajeAgenta(const std::string &ime_agenta){
    for(int i=0;i<p.size();i++) if(p[i]!=nullptr)if(p[i]->DajImeAgenta()==ime_agenta){p[i]=nullptr; p.erase(p.begin()+i);}
    
}
void ObrisiProdajeNaDatum(const Datum &datum){
    for(int i=0;i<p.size();i++) if(p[i]!=nullptr)if(poredidvadatuma(p[i]->DajDatumProdaje(),datum)){ p[i]=nullptr;  p.erase(p.begin()+i);}
}
void IspisiProdajeNaDatum(const Datum &datum){
    for(int i=0;i<p.size();i++) if(p[i]!=nullptr)if(poredidvadatuma(p[i]->DajDatumProdaje(),datum)) p[i]->Ispisi();
}
void IspisiSveProdaje() const{
    std::vector<Prodaja> pr;
    for(int i=0;i<p.size();i++) pr.emplace_back(*p[i]);
...
void IspisiSveProdaje2() const{
    ...
    for(int i=0;i<p.size();i++){
      ...
      p[i]->Ispisi();  j++; std::cout<<std::endl;
        }
    } 
}
...
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja){
    ...
    try{
        for(int i=0;i<spisak_prodaja.size();i++){
            p.emplace_back(std::make_shared<Prodaja>(*it++));
        }
    }
    catch(...){
        p.resize(0);
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje,double cijena_stana, const Datum &datum_prodaje, const Kupac &kupac_stana,const Stan &kupljeni_stan){
    p.emplace_back(std::make_shared<Prodaja>(Prodaja(ime_agenta_prodaje,cijena_stana,datum_prodaje,kupac_stana,kupljeni_stan)));
