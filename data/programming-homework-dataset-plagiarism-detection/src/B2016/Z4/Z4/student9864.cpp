...
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja)
{
    ...
    while(it != spisak_prodaja.end())
        {
            std::shared_ptr<Prodaja> pomocni = std::make_shared<Prodaja> (*it);
            niz.push_back(pomocni);
...
Prodaje::Prodaje(const Prodaje &prodaje)
{
    niz.resize(0);
        ...
        for (int i{};i<prodaje.niz.size();i++)
            {
                std::shared_ptr<Prodaja> pomocni = std::make_shared<Prodaja> (*prodaje.niz[i]);
                niz.push_back(pomocni);
            }
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, const Datum &datum_prodaje, 
        const Kupac &kupac_stana, const Stan &kupljeni_stan)
{
    std::shared_ptr<Prodaja> pomocni = std::make_shared<Prodaja> (ime_agenta_prodaje,cijena_stana,datum_prodaje,kupac_stana,kupljeni_stan);
    niz.push_back(pomocni);
}
...
void Prodaje::RegistrirajProdaju(const std::string &ime_agenta_prodaje, double cijena_stana, int dan_prodaje, int mjesec_prodaje, 
        ...
        int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata)
{
    std::shared_ptr<Prodaja> pomocni = std::make_shared<Prodaja> (ime_agenta_prodaje,cijena_stana,dan_prodaje,mjesec_prodaje,godina_prodaje,
...
{
    std::shared_ptr<Prodaja> pomocni = std::make_shared<Prodaja> (*prodaja);
    niz.push_back(pomocni);
}
...
{
    int rezultat = std::count_if(niz.begin(),niz.end(), [datum] (std::shared_ptr<Prodaja> pok)
    {
...
{
    int rezultat = std::count_if(niz.begin(),niz.end(), [ime_agenta] (std::shared_ptr<Prodaja> pok)
    {
...
Prodaja &Prodaje::DajNajranijuProdaju()
{
    ...
    std::shared_ptr<Prodaja> rezultat = *std::min_element(niz.begin(),niz.end(), [] (std::shared_ptr<Prodaja> p1, std::shared_ptr<Prodaja> p2)
    {
...
Prodaja &Prodaje::DajNajskupljuProdaju()
{
    ...
    std::shared_ptr<Prodaja> rezultat = *std::max_element(niz.begin(),niz.end(), [] (std::shared_ptr<Prodaja> p1, std::shared_ptr<Prodaja> p2)
    {
...
{
    if (redni_broj<1 || redni_broj>niz.size())
...
void Prodaje::ObrisiNajranijuProdaju()
{
    ...
    Prodaja trazeni = DajNajranijuProdaju();
    std::shared_ptr<Prodaja> brisanje = nullptr;
    
    for (int i{};i<niz.size();i++)
        {
            if (trazeni.DajImeAgenta() == niz[i]->DajImeAgenta())
                {
                    if (trazeni.DajImeKupca() == niz[i]->DajImeKupca())
                        {
                            Datum d1 = trazeni.DajDatumProdaje();
                            Datum d2 = niz[i]->DajDatumProdaje();
                            
                            if (d1.DajDan() == d2.DajDan() && d1.DajMjesec() == d2.DajMjesec() && d1.DajMjesec() == d2.DajMjesec())
                                {
                                    if (trazeni.DajCijenuStana() == niz[i]->DajCijenuStana())
                                        brisanje = niz[i];
                                }
                        }
                }
    ...
    std::vector<std::shared_ptr<Prodaja>> pomocna;
    for (int i{};i<niz.size();i++)
        {
            if (niz[i] != nullptr)
                {
                    std::shared_ptr<Prodaja> upis = std::make_shared<Prodaja> (*niz[i]);
                    pomocna.push_back(upis);
                }
        }
     
    niz.resize(0);  
    niz.resize(pomocna.size());
...
void Prodaje::IspisiProdajeNaDatum(const Datum &datum) const
{
    ...
    std::sort(ispis.niz.begin(),ispis.niz.end(), [] (std::shared_ptr<Prodaja> p1, std::shared_ptr<Prodaja> p2)
     {
                                        ...
                                        while(true)
                                            {
                                                ...
                                                try
                                                {
                                                    ...
                                                    P.RegistrirajProdaju(std::make_shared<Prodaja> (upis));
