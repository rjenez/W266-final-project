...
class Prodaje {
    std::vector<std::shared_ptr<Prodaja>> prodaje2;
    public:
    explicit Prodaje() : prodaje2(0) {  }
...
void Prodaje::IspisiSveProdaje() const {
    std::sort(prodaje2.begin(), prodaje2.end(), [this](std::shared_ptr<Prodaja> p, std::shared_ptr<Prodaja> q) {
    ...
    for(int i = 0; i < prodaje2.size(); i++) prodaje2[i] -> Ispisi();
}
void Prodaje::IspisiProdajeNaDatum(const Datum &datum) const {
    std::sort(prodaje2.begin(), prodaje2.end(), [](std::shared_ptr<Prodaja> p, std::shared_ptr<Prodaja> q) {
        return p->DajImeKupca() > q->DajImeKupca();
    }); for(int i = 0; i < prodaje2.size(); i++) {
...
void Prodaje::ObrisiProdajeNaDatum(const Datum &datum) {
    std::remove_if(prodaje2.begin(), prodaje2.end(), [datum](std::make_shared<Prodaja> pok) { auto p = pok->DajDatumProdaje(); 
    return (datum.DajDan() == p->DajDan() && datum.DajMjesec() == p->DajMjesec() && datum.DajGodinu() == p->DajGodinu());});
}
void Prodaje::ObrisiProdajeAgenta(const std::string &ime_agenta) {
    std::remove_if(prodaje2.begin(), prodaje2.end(), [ime_agenta](std::make_shared<Prodaja> pok) { return pok->DajImeAgenta() == ime_agenta;});
}
void Prodaje::ObrisiNajranijuProdaju() {
    if(prodaje2.size() == 0) throw std::range_error("Prazna kolekcija");
    std::remove_if(prodaje2.begin(), prodaje2.end(),  [this](std::shared_ptr<Prodaja> pok) { return pok == std::make_shared<Prodaja>(DajNajranijuProdaju());});
}
void Prodaje::IsprazniKolekciju() {
    for(int i = 0; i < prodaje2.size(); i++) prodaje2 [i] = nullptr;
}
Prodaja Prodaje::DajNajskupljuProdaju() const {
    if(prodaje2.size() == 0) throw std::domain_error("Nema registriranih prodaja");
    return **(std::max_element(prodaje2.begin(), prodaje2.end(), [](std::shared_ptr<Prodaja> p, std::shared_ptr<Prodaja> q) { return p->DajCijenuStana() > q->DajCijenuStana();}));
}
Prodaja &Prodaje::DajNajskupljuProdaju() {
    if(prodaje2.size() == 0) throw std::domain_error("Nema registriranih prodaja");
    Prodaja &a = **(std::max_element(prodaje2.begin(), prodaje2.end(), [](std::shared_ptr<Prodaja> p, std::shared_ptr<Prodaja> q) {
        return p->DajCijenuStana() > q->DajCijenuStana();
    })); return a;
}
Prodaja Prodaje::DajNajranijuProdaju() const {
    if(prodaje2.size() == 0) throw std::domain_error("Nema registriranih prodaja");
    return **(std::min_element(prodaje2.begin(), prodaje2.end(), [](std::shared_ptr<Prodaja> p, std::shared_ptr<Prodaja> q) {
        return ProdatPrije(*p, *q);
    }));
}
Prodaja &Prodaje::DajNajranijuProdaju() {
    if(prodaje2.size() == 0) throw std::domain_error("Nema registriranih prodaja");
    Prodaja &a = **(std::min_element(prodaje2.begin(), prodaje2.end(), [](std::shared_ptr<Prodaja> p, std::shared_ptr<Prodaja> q) {
        return ProdatPrije(*p, *q);
    })); return a;
}
Prodaja &Prodaje::DajProdaju(int n) {
    if(n > prodaje2.size() || n < 0) throw std::domain_error("Neispravan indeks prodaje");
    Prodaja &a = *prodaje2[n - 1];
    return a;
}
int Prodaje::DajBrojProdajaOdAgenta(const std::string &ime_agenta) const {
    return std::count_if(prodaje2.begin(), prodaje2.end(), [ime_agenta](std::shared_ptr<Prodaja> pok) {
        return (pok->DajImeAgenta() == ime_agenta);
    });
}
int Prodaje::DajBrojProdajaNaDatum(const Datum &datum) const {
    return std::count_if(prodaje2.begin(), prodaje2.end(), [datum](std::shared_ptr<Prodaja> pok) { auto pom = pok->DajDatumProdaje(); 
    return (pom.DajDan() == datum.DajDan() && pom.DajMjesec() == datum.DajMjesec() && pom.DajGodinu() == datum.DajGodinu()); });
}
Prodaje &Prodaje::operator =(Prodaje &&prodaje) {
    prodaje2.resize(prodaje.prodaje2.size());
    for(int i = 0; i < prodaje2.size(); i++)
        prodaje2[i] = std::make_shared<Prodaja>(*prodaje.prodaje2[i]);
    for(int i = 0; i < prodaje2.size(); i++)
        prodaje.prodaje2[i] = nullptr;
    prodaje.prodaje2.resize(0);
    return *this;
}
Prodaje &Prodaje::operator =(const Prodaje &prodaje) {
    if(&prodaje != this) {
        if(prodaje2.size() < prodaje.prodaje2.size())
        {
            for(int i = 0; i < prodaje2.size(); i++)
                prodaje2[i] = std::make_shared<Prodaja>(*prodaje.prodaje2[i]);
            for(int i = prodaje2.size(); i < prodaje.prodaje2.size(); i++)
                prodaje2.push_back(std::make_shared<Prodaja> (*prodaje.prodaje2[i]));
        }
        else {
            for(int i = 0; i < prodaje.prodaje2.size(); i++)
                prodaje2[i] = std::make_shared<Prodaja>(*prodaje.prodaje2[i]);
            for(int i = prodaje.prodaje2.size(); i < prodaje2.size(); i++)
                prodaje2[i] = nullptr;
        }
    } return *this;
}
...
Prodaje::Prodaje(std::initializer_list<Prodaja> spisak_prodaja)
{
    ...
    for(const auto &pok: spisak_prodaja)
        prodaje2.push_back(std::make_shared<Prodaja>(pok));
}    
void Prodaje::RegistrirajProdaju(Prodaja *prodaja) {
    prodaje2.push_back(std::make_shared<Prodaja>(*prodaja));
}
...
const Datum &datum_rodjenja_kupca, const std::string &adresa_stana, int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata)
{
    prodaje2.push_back(std::make_shared<Prodaja>(ime_agenta_prodaje, cijena_stana, dan_prodaje, mjesec_prodaje, godina_prodaje, ime_kupca, datum_rodjenja_kupca,
...
const Stan &kupljeni_stan) {
    prodaje2.push_back(std::make_shared<Prodaja>(ime_agenta_prodaje, cijena_stana, datum_prodaje, kupac_stana, kupljeni_stan));
} /*
Prodaje::~Prodaje() {
