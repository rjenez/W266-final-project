...
class Prodaje
{
    vector<shared_ptr<Prodaja>> prodaja;
    
    public:
        explicit Prodaje() : prodaja(0) {}
        Prodaje(initializer_list<Prodaja> spisak_prodaja);
        ~Prodaje(){}
        Prodaje(const Prodaje &prodaje);
        Prodaje(Prodaje &&prodaje) : prodaja(move(prodaje.prodaja)) { }
        Prodaje &operator =(const Prodaje &prodaje);
        Prodaje &operator =(Prodaje &&prodaje) { prodaja=move(prodaje.prodaja); return *this; }
        ...
        int DajBrojProdaja() const { return prodaja.size(); }
...
void Stan::Ispisi() const
{
    ...
    if(namjesten) cout << " Kvadratura stana je " << fixed << setprecision(2) << kvadratura << " (m^2) i stan je namjesten." << endl;
    else cout << " Kvadratura stana je " << fixed << setprecision(2) << kvadratura << " (m^2) i stan nije namjesten." << endl;
}
...
Prodaje::Prodaje(initializer_list<Prodaja> spisak_prodaja)
{
    ...
    {
        prodaja.push_back(make_shared<Prodaja>(*it));
    }   
}

Prodaje::Prodaje(const Prodaje &prodaje) : prodaja(prodaje.prodaja.size())
{
    try 
	{
		for(int i = 0; i < prodaje.prodaja.size(); i++)
			Prodaje::prodaja[i] = make_shared<Prodaja>(*prodaje.Prodaje::prodaja[i]);
	}
	catch(...) 
	{
		throw;
...
{
	if(prodaje.prodaja.size()>prodaja.size())
	{
		try 
		{
			for(int i = prodaja.size(); i < prodaje.prodaja.size(); i++)
				prodaja.push_back(make_shared<Prodaja>(*prodaje.Prodaje::prodaja[i]));
		}
		catch(...) 
		{
			throw;
	...
	else prodaja.resize(prodaje.prodaja.size());
	
	for(int i=0; i<prodaja.size(); i++) 
		*Prodaje::prodaja[i] = *prodaje.Prodaje::prodaja[i];
...
void Prodaje::RegistrirajProdaju(const string &ime_agenta_prodaje, double cijena_stana, const Datum &datum_prodaje, const Kupac &kupac_stana, const Stan &kupljeni_stan)
{
    try
    {
        prodaja.push_back(make_shared<Prodaja>(Prodaja(ime_agenta_prodaje, cijena_stana, datum_prodaje, kupac_stana, kupljeni_stan)));
    }
    catch(...)
    {
...
void Prodaje::RegistrirajProdaju(const string &ime_agenta_prodaje, double cijena_stana, int dan_prodaje, int mjesec_prodaje, int godina_prodaje, string &ime_kupca, const Datum &datum_rodjenja_kupca, const string &adresa_stana, int sprat_stana, int broj_soba, bool namjesten_stan, double broj_kvadrata)
{
    try
    {
        prodaja.push_back(make_shared<Prodaja>(Prodaja(ime_agenta_prodaje, cijena_stana, Datum(dan_prodaje, mjesec_prodaje, godina_prodaje), Kupac(ime_kupca, datum_rodjenja_kupca), Stan(adresa_stana, sprat_stana, broj_soba, namjesten_stan, broj_kvadrata))));
    }
    catch(...)
    {
        cout << "Nedovoljno memorije" << endl;
...
void Prodaje::RegistrirajProdaju(Prodaja *prodaja)
{
    try
    {
        Prodaje::prodaja.push_back(make_shared<Prodaja>(*prodaja));
    }
    catch(...)
    {
        cout << "Nedovoljno memorije" << endl;
...
{
    return count_if(&prodaja[0], &prodaja[0]+prodaja.size(), [datum](shared_ptr<Prodaja> p) {  if(p->DajDatumProdaje().DajDan()==datum.DajDan() && p->DajDatumProdaje().DajMjesec()==datum.DajMjesec() && p->DajDatumProdaje().DajGodinu()==datum.DajGodinu()) return true; return false; });
}

int Prodaje::DajBrojProdajaOdAgenta(const string &ime_agenta) const
{
    return count_if(&prodaja[0], &prodaja[0]+prodaja.size(), [ime_agenta](shared_ptr<Prodaja> p) {  if(ime_agenta==p->DajImeAgenta()) return true; return false; });
}

Prodaja &Prodaje::DajProdaju(int redni_broj)
{
    if(redni_broj<=0 || redni_broj>prodaja.size()) throw std::range_error("Ne postoji prodaja pod tim rednim brojem!");
    return *prodaja[redni_broj-1];
}

Prodaja &Prodaje::DajNajranijuProdaju()
{
    return **min_element(&prodaja[0], &prodaja[0]+prodaja.size(), [](shared_ptr<Prodaja> p1, shared_ptr<Prodaja> p2) { if(ProdatPrije(*p1, *p2)) return true;  return false;});
}
...
{
    return **max_element(&prodaja[0], &prodaja[0]+prodaja.size(), [](shared_ptr<Prodaja> p1, shared_ptr<Prodaja> p2) { if(SkupljiStan(*p2, *p1)) return true;  return false;});
}
...
{
    if(prodaja.size()==0) throw std::range_error("Prazna kolekcija");
   
    auto pok(min_element(&prodaja[0], &prodaja[0]+prodaja.size(), [](shared_ptr<Prodaja> p1, shared_ptr<Prodaja> p2) { if(ProdatPrije(*p1, *p2)) return true;  return false;}));
    for(int i=0; i<prodaja.size(); i++)
    {
        if(prodaja[i]==*pok)
        {
            prodaja.erase(prodaja.begin()+i);
            return;
        }
    }
}
...
void Prodaje::ObrisiProdajeAgenta(const string &ime_agenta)
{
    for(int i=0; i<prodaja.size(); i++)
    {
        if(prodaja[i]->DajImeAgenta()==ime_agenta)
        {
            prodaja.erase(prodaja.begin()+i);
            i--;
        }
...
void Prodaje::ObrisiProdajeNaDatum(const Datum &datum)
{
    for(int i=0; i<prodaja.size(); i++)
    {
        if(prodaja[i]->DajDatumProdaje().DajDan()==datum.DajDan() && prodaja[i]->DajDatumProdaje().DajMjesec()==datum.DajMjesec() && prodaja[i]->DajDatumProdaje().DajGodinu()==datum.DajGodinu())
        {
            prodaja.erase(prodaja.begin()+i);
            i--;
        }
...
void Prodaje::IspisiProdajeNaDatum(const Datum &datum) const
{
    ...
    for(int i=0; i<prodaja.size(); i++)
    {
        if(prodaja[i]->DajDatumProdaje().DajDan()==datum.DajDan() && prodaja[i]->DajDatumProdaje().DajMjesec()==datum.DajMjesec() && prodaja[i]->DajDatumProdaje().DajGodinu()==datum.DajGodinu())
        {
            pom.prodaja.push_back(make_shared<Prodaja>(*prodaja[i]));
        }
    }
    sort(&pom.prodaja[0], &pom.prodaja[0]+pom.prodaja.size(), [] (shared_ptr<Prodaja> p1, shared_ptr<Prodaja> p2) { if(p1->DajImeKupca() < p2->DajImeKupca()) return true; return false; });
    for(int i=0; i<pom.prodaja.size(); i++)
        pom.prodaja[i]->Ispisi();
}
...
void Prodaje::IspisiSveProdaje() const
{
    ...
    sort(&pom.prodaja[0], &pom.prodaja[0]+pom.prodaja.size(), [] (shared_ptr<Prodaja> p1, shared_ptr<Prodaja> p2)
    { 
                            ...
                            while(1)
                            {
                                ...
                                else if(opcija_3_2_3==2)
                                {
                                    ...
                                    try
                                    {
                                        ...
                                        cin.ignore(1000, '\n');
                                        cout << "Unesite datum rodjenja (formata d/m/g): ";
                                        cin >> dan3 >> znak >> mjesec3 >> znak >> godina3;
