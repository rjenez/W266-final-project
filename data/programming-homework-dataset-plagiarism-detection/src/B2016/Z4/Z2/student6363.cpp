...
class Knjiga{
    ...
    Korisnik *DajKodKogaJe() const {return poknakor;}
    void ZaduziKnjigu (Korisnik &korisnik) { poknakor=&korisnik; }
...
//    Biblioteka(const Biblioteka &v);
 ...
 void Biblioteka::RegistrirajNovogKorisnika(int indeks, std::string ime1,std::string prezime1, std::string adresa1, std::string telefon1 ){
        ...
        try{
        korisnik=new Korisnik{ime1, prezime1, adresa1,telefon1};
        korisnici[indeks]=korisnik;
        }
        catch(...)
        {
            std::cout<<"Problemi sa memorijom";
            delete korisnik; korisnik=nullptr;
        }
    }
 ...
 void Biblioteka::RegistrirajNovuKnjigu(int broj, std::string naslov, std::string imepisca, std::string zanrzanr, int godina){
     ...
     try{
         knjiga=new Knjiga(naslov, imepisca, zanrzanr,godina);
         knjige[broj]=knjiga;
     }
     catch(...)
     {
         std::cout<<"Problemi sa memorijom";
         delete knjiga; knjiga=nullptr;
     }
 }    
