...
//				    || ((r==mr)&&mxyIsDefined&&(M.at(y).at(x)==mxy)&&(x<mx))
//				    || ((r==mr)&&mxyIsDefined&&(M.at(y).at(x)==mxy)&&(x==mx)&&(y<my))
				  ){
					mr=r;
					mx=x;
					my=y;
...
//	std::cout<<std::endl<<"Zavrsio";
	...
	{
	    if (m==0 && n==0) {
			//std::cout<<"0 matrica"; //ovdje dodati ako nesto treba ispisati u ovom slucaju
		}
		else
		{
